...
// ═══════════════════════════════════════════════════════════
import { state } from './app.js?v=20260714c';
import { invokeEdgeFunction, showToast, supabase } from './api.js?v=20260714c';
import { esc, str } from './utils.js?v=20260714c';
...
export function renderPayroll() {
  ...
  if (!_employeesLoaded || !_paymentsLoaded || !state.trackerLoaded) {
    Promise.all([
      ...
      state.trackerLoaded ? Promise.resolve() : import('./lead-tracker.js?v=20260714c').then(m => m.loadTrackerEntries()),
