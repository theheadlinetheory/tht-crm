...
import { state, pendingWrites, settingsOpen, setSettingsOpen, settingsTab, setSettingsTab,
         settingsDraft, setSettingsDraft, clientsSubTab, setClientsSubTab } from './app.js?v=20260918110319';
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, CLIENT_SOP_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, CLIENT_INFO_SHEET_ID, SEQUENCE_TEMPLATES } from './config.js?v=20260918110319';
import { render } from './render.js?v=20260918110319';
import { apiPost, apiGet, sbBatchUpdateClients, sbUpdateClient, sbSaveSettings, camelToSnake, supabase, invokeEdgeFunction, showToast, sbDeleteFile, sbGetSignedUrl } from './api.js?v=20260918110319';
import { renderRoutingRules } from './routing-rules.js?v=20260918110319';
import { esc, str, svgIcon } from './utils.js?v=20260918110319';
import { isAdmin, isEmployee, currentUser, loadAllUsers, updateUserRole, updateUserName, updateUserTagColor, updateUserPhoto, deleteUser, getOwnerColor as authGetOwnerColor, TAG_PALETTE } from './auth.js?v=20260918110319';
import { lookupClientInfo } from './client-info.js?v=20260918110319';
import { findPolygonForClient, invalidateServiceAreaCache } from './maps.js?v=20260918110319';
import { renderDocumentsSection, initDocumentHandlers } from './documents.js?v=20260918110319';
import { DEFAULT_BOOKING_SMS_TEMPLATE } from './booking-sms.js?v=20260918110319';
import { renderRetainerBilling } from './retainer-billing.js?v=20260918110319';
import { showClientEndPicker, clearClientEnd } from './client-end.js?v=20260918110319';
import { setupBadge, setupBanner } from './client-setup-status.js?v=20260918110319';
...
export function refreshSettingsBody(){
  ...
  if(settingsTab==='pipeline') h=renderPipelineConfigSettings();
    ...
    if (!window._dialerFieldsLoaded) {
      ...
      import('./number-health.js?v=20260918110319').then(m => m.loadNumberHealth().then(() => refreshSettingsBody())).catch(() => {});
    }
...
window.markSelectedPaid = async function(){
  ...
  try{
    const { sbUpdateTrackerEntry } = await import('./api.js?v=20260918110319');
...
window.restoreClient = async function(clientId) {
  ...
  try {
    const { createSmartleadPortal } = await import('./smartlead-portal.js?v=20260918110319');
...
window.createClientPortal = async function(clientId) {
  const c = state.clients.find(x => str(x.id) === str(clientId));
  if (!c) return;
  const { portalEmail, createSmartleadPortal } = await import('./smartlead-portal.js?v=20260918110319');
