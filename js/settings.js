...
import { state, pendingWrites, settingsOpen, setSettingsOpen, settingsTab, setSettingsTab,
         settingsDraft, setSettingsDraft, clientsSubTab, setClientsSubTab } from './app.js?v=20260905104727';
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, CLIENT_SOP_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, CLIENT_INFO_SHEET_ID, SEQUENCE_TEMPLATES } from './config.js?v=20260905104727';
import { render } from './render.js?v=20260905104727';
import { apiPost, apiGet, sbBatchUpdateClients, sbUpdateClient, sbSaveSettings, camelToSnake, supabase, invokeEdgeFunction, showToast, sbDeleteFile, sbGetSignedUrl } from './api.js?v=20260905104727';
import { renderRoutingRules } from './routing-rules.js?v=20260905104727';
import { esc, str, svgIcon } from './utils.js?v=20260905104727';
import { isAdmin, isEmployee, currentUser, loadAllUsers, updateUserRole, updateUserName, updateUserTagColor, updateUserPhoto, deleteUser, getOwnerColor as authGetOwnerColor, TAG_PALETTE } from './auth.js?v=20260905104727';
import { lookupClientInfo } from './client-info.js?v=20260905104727';
import { findPolygonForClient, invalidateServiceAreaCache } from './maps.js?v=20260905104727';
import { renderDocumentsSection, initDocumentHandlers } from './documents.js?v=20260905104727';
import { DEFAULT_BOOKING_SMS_TEMPLATE } from './booking-sms.js?v=20260905104727';
import { renderRetainerBilling } from './retainer-billing.js?v=20260905104727';
import { showClientEndPicker, clearClientEnd } from './client-end.js?v=20260905104727';
import { setupBadge, setupBanner } from './client-setup-status.js?v=20260905104727';
...
export function renderSettingsPanel(){
  ...
  else if(settingsTab==='campaigns'){ h+=renderCampaignAssignSettings(); h+=renderRoutingRules((state.assignableUsers||[]).map(u=>u.name).filter(Boolean)); }
...
export function refreshSettingsBody(){
  ...
  if(settingsTab==='pipeline') h=renderPipelineConfigSettings();
    ...
    if (!window._dialerFieldsLoaded) {
      ...
      import('./number-health.js?v=20260905104727').then(m => m.loadNumberHealth().then(() => refreshSettingsBody())).catch(() => {});
    }
  ...
  // Preserve scroll on in-place refreshes (add/delete/reorder rows, async
  // reloads). Only jump to top when the settings TAB actually changes.
  const _prevScroll = body.scrollTop;
  const _tabChanged = window._lastSettingsTab !== settingsTab;
  window._lastSettingsTab = settingsTab;
  body.innerHTML=h;
  body.scrollTop = _tabChanged ? 0 : _prevScroll;
  setupSettingsDrag();
...
function renderClientsSettings(){
  ...
  for(const c of _activeClients){
    ...
    h+=`<div class="client-card-accordion" data-client-id="${esc(c.id)}" style="margin-bottom:8px;background:var(--bg);border:1px solid var(--border);border-radius:10px;overflow:hidden">
      ...
      ${isAdmin()?`<div style="margin-bottom:8px;padding:10px;background:#faf5ff;border:1px solid #e9d5ff;border-radius:8px">
        ...
        <div style="margin-bottom:6px">
          ...
          <input type="text" placeholder="e.g. ve9EPM428h8vShlRW1KT" value="${esc(str(c.ghlLocationId))}"
            oninput="updateClientField('${esc(c.id)}','ghlLocationId',this.value);debouncedAutoSave()"
            style="width:100%;box-sizing:border-box;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:12px;font-family:var(--font);background:var(--card);color:var(--text);margin-top:3px">
        </div>
        <div style="margin-bottom:6px;position:relative">
          ...
          <input type="password" id="ghl-key-${esc(c.id)}" placeholder="pit-..." value="${esc(str(c.ghlApiKey))}"
            oninput="updateClientField('${esc(c.id)}','ghlApiKey',this.value);debouncedAutoSave()"
            style="width:100%;box-sizing:border-box;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:12px;font-family:var(--font);background:var(--card);color:var(--text);margin-top:3px">
        ...
        <div style="display:flex;gap:6px;margin-bottom:2px">
          <div style="flex:1">
            ...
            <input type="text" placeholder="e.g. abc123..." value="${esc(str(c.ghlPipelineId))}"
              oninput="updateClientField('${esc(c.id)}','ghlPipelineId',this.value);debouncedAutoSave()"
              style="width:100%;box-sizing:border-box;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:11px;font-family:var(--font);background:var(--card);color:var(--text);margin-top:3px">
          </div>
          <div style="flex:1">
            ...
            <input type="text" placeholder="e.g. xyz789..." value="${esc(str(c.ghlStageId))}"
              oninput="updateClientField('${esc(c.id)}','ghlStageId',this.value);debouncedAutoSave()"
              style="width:100%;box-sizing:border-box;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:11px;font-family:var(--font);background:var(--card);color:var(--text);margin-top:3px">
          </div>
        </div>
      </div>`:''}
      ...
      <div style="margin-bottom:8px">
        ...
        ${(()=>{
          ...
          if(pm){
            ...
            return '<div style="display:flex;align-items:center;gap:6px;margin-top:3px;padding:6px 10px;background:#dcfce7;border:1px solid #86efac;border-radius:6px;font-size:11px;color:#166534;justify-content:space-between"><span style="display:flex;align-items:center;gap:6px"><span class="sa-badge sa-in" style="width:14px;height:14px;font-size:8px">&#10003;</span> Polygon configured ('+(coordCount/1024).toFixed(1)+'KB)'+(hasDb?' \u2014 stored in DB':'')+'</span><span style="display:flex;gap:4px"><button onclick="downloadKml(\''+esc(c.id)+'\')" style="font-size:10px;padding:3px 8px;border:1px solid #86efac;border-radius:4px;background:#fff;color:#166534;cursor:pointer;font-family:var(--font);font-weight:600">Download KML</button><button onclick="uploadKml(\''+esc(c.id)+'\')" style="font-size:10px;padding:3px 8px;border:1px solid #86efac;border-radius:4px;background:#fff;color:#166534;cursor:pointer;font-family:var(--font);font-weight:600">Replace KML</button></span></div>';
          }
      ...
      <!-- Offboarding (2026-09-05). Red and at the very bottom so it is never
           hit by accident; the picker it opens asks for the last day of service
           and why (client-end.js), which is what level 06 runs on. -->
      <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;align-items:center;gap:10px">
        <span style="font-size:11px;color:#9ca3af">Client leaving? Record when and why — they move to Past Clients.</span>
        <button onclick="deactivateClient('${esc(c.id)}')" title="Offboard this client — asks for the last day of service and the reason"
          style="padding:8px 14px;background:#fef2f2;color:#dc2626;border:1px solid #fecaca;border-radius:7px;font-size:12px;font-weight:700;cursor:pointer">Offboard client</button>
      </div>
      </div>
    </div>`;
  }
  ...
  input.onchange=async()=>{
    ...
    try {
      ...
      try {
        ...
        const storagePath='service-areas/'+slug+'.kml';
        try { await sbDeleteFile(storagePath); } catch(_){}
        const blob=new Blob([text],{type:'application/vnd.google-earth.kml+xml'});
        const kmlFile=new File([blob],slug+'.kml',{type:'application/vnd.google-earth.kml+xml'});
        await supabase.storage.from('client-documents').upload(storagePath,kmlFile,{cacheControl:'3600',upsert:true});
      }
...
export async function downloadKml(clientId){
  const c=state.clients.find(x=>str(x.id)===str(clientId));
  if(!c) return;
  const slug=c.name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/-+$/,'');
  const storagePath='service-areas/'+slug+'.kml';
  try {
    const url=await sbGetSignedUrl(storagePath,300);
    const a=document.createElement('a');
    a.href=url;
    a.download=slug+'.kml';
    a.click();
  } catch(e){
    console.error('KML download error:',e);
    showToast('No KML file found in storage for '+c.name+'. Try re-uploading.','error');
  }
}
...
window.markSelectedPaid = async function(){
  ...
  try{
    const { sbUpdateTrackerEntry } = await import('./api.js?v=20260905104727');
...
window.createClientPortal = async function(clientId) {
  const c = state.clients.find(x => str(x.id) === str(clientId));
  if (!c) return;
  const { portalEmail, createSmartleadPortal } = await import('./smartlead-portal.js?v=20260905104727');
