...
import { state, pendingWrites, settingsOpen, setSettingsOpen, settingsTab, setSettingsTab,
         settingsDraft, setSettingsDraft, clientsSubTab, setClientsSubTab } from './app.js?v=20260515c';
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, CLIENT_SOP_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, CLIENT_INFO_SHEET_ID, SEQUENCE_TEMPLATES } from './config.js?v=20260515c';
import { render } from './render.js?v=20260515c';
import { apiPost, apiGet, sbBatchUpdateClients, sbUpdateClientConfig, sbSaveSettings, camelToSnake, supabase, invokeEdgeFunction, showToast } from './api.js?v=20260515c';
import { esc, str, svgIcon } from './utils.js?v=20260515c';
import { isAdmin, isEmployee, currentUser, loadAllUsers, updateUserRole, updateUserClient, updateUserName, updateUserEmail, deleteFirebaseUser, getOwnerColor as authGetOwnerColor, TAG_PALETTE, db } from './auth.js?v=20260515c';
import { lookupClientInfo, getClientConfig, loadClientConfig } from './client-info.js?v=20260515c';
import { findPolygonForClient } from './maps.js?v=20260515c';
...
export async function createNewUser(){
  ...
  try {
    const { auth } = await import('./auth.js?v=20260515c');
...
window.markSelectedPaid = async function(){
  ...
  try{
    const { sbUpdateTrackerEntry } = await import('./api.js?v=20260515c');
