...
import { state, pendingWrites, settingsOpen, setSettingsOpen, settingsTab, setSettingsTab,
         settingsDraft, setSettingsDraft, clientsSubTab, setClientsSubTab } from './app.js?v=20260715a';
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, CLIENT_SOP_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, CLIENT_INFO_SHEET_ID, SEQUENCE_TEMPLATES } from './config.js?v=20260715a';
import { render } from './render.js?v=20260715a';
import { apiPost, apiGet, sbBatchUpdateClients, sbUpdateClient, sbSaveSettings, camelToSnake, supabase, invokeEdgeFunction, showToast, sbDeleteFile, sbGetSignedUrl } from './api.js?v=20260715a';
import { esc, str, svgIcon } from './utils.js?v=20260715a';
import { isAdmin, isEmployee, currentUser, loadAllUsers, updateUserRole, updateUserClient, updateUserName, updateUserEmail, deleteFirebaseUser, getOwnerColor as authGetOwnerColor, TAG_PALETTE, db } from './auth.js?v=20260715a';
import { lookupClientInfo } from './client-info.js?v=20260715a';
import { findPolygonForClient } from './maps.js?v=20260715a';
...
export function refreshSettingsBody(){
  ...
  if(settingsTab==='pipeline') h=renderPipelineConfigSettings();
    ...
    if (!window._dialerFieldsLoaded) {
      ...
      import('./number-health.js?v=20260715a').then(m => m.loadNumberHealth().then(() => refreshSettingsBody())).catch(() => {});
    }
...
export async function createNewUser(){
  ...
  try {
    const { auth } = await import('./auth.js?v=20260715a');
...
window.markSelectedPaid = async function(){
  ...
  try{
    const { sbUpdateTrackerEntry } = await import('./api.js?v=20260715a');
