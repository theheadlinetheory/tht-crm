...
// ═══════════════════════════════════════════════════════════
import { API_URL, SUPABASE_URL, SUPABASE_ANON_KEY } from './config.js?v=20260620e';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
export { supabase };
import { state, store, pendingWrites, failedWriteQueue, pendingDealFields, deletedDealIds, deletedActivityIds, completedActivityIds, deletedClientIds, inFlightActivityIds } from './app.js?v=20260620e';
  ...
  if(data && !data.error){
    ...
    if(data.clients && Array.isArray(data.clients) && data.clients.length>0){
      ...
      for(const c of state.clients){
        if(!c.calendlyUrl){
          const { getClientConfig } = await import('./client-info.js?v=20260620e');
    ...
    if(data.appointments && Array.isArray(data.appointments)){
      const { getToday } = await import('./utils.js?v=20260620e');
    ...
    const { runServiceAreaChecks } = await import('./maps.js?v=20260620e');
    ...
    if((isAdmin()||isEmployee()) && !state.archiveLoaded){
      const { loadArchive } = await import('./archive.js?v=20260620e');
      loadArchive(true);
    }
  } else {
    if(state.deals.length===0){
      const { getTestData } = await import('./config.js?v=20260620e');
...
export async function initialSync(isStartup) {
  ...
  try {
    ...
    render();
    if (isStartup) {
      import('./dashboard.js?v=20260620e').then(m => m.clearDashboardArchiveCache && m.clearDashboardArchiveCache()).catch(() => {});
    }
    ...
    if (savedSettings && Object.keys(savedSettings).length > 0) {
      const { applySettings } = await import('./settings.js?v=20260620e');
    ...
    if (!_cachedIsAdmin) {
      const { isAdmin: _isAdmin } = await import('./auth.js?v=20260620e');
    ...
    if (isStartup) {
      import('./activities.js?v=20260620e').then(m => m.replayPendingActivities && m.replayPendingActivities()).catch(() => {});
    }
...
const DEALS_LIGHT_COLS = 'id,company,contact,email,phone,value,stage,pipeline,flag,notes,sl_lead_id,sl_campaign_id,campaign_name,lead_category,created_at,updated_at,website,location,smartlead_url,forwarded_at,mobile_phone,pushed_to_tracker,pushed_to_ghl,address,client_stage,booked_date,booked_time,booked_for,booked_timezone,cal_name,cal_email,cal_notes,has_new_reply,has_new_text,owner_override,job_title,linkedin_url,passoff_instructions,passoff_sent_at,suggested_updates,contact2,contact3,phone2,phone3,title2,title3,email2,email3,email4,reply_snippet,retarget_campaign,retarget_date,retarget_status,retarget_count,auto_followup_started_at';
