...
// ═══════════════════════════════════════════════════════════
import { API_URL, SUPABASE_URL, SUPABASE_ANON_KEY } from './config.js?v=20260531e';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
export { supabase };
import { state, store, pendingWrites, failedWriteQueue, pendingDealFields, deletedDealIds, deletedActivityIds, completedActivityIds, deletedClientIds, inFlightActivityIds } from './app.js?v=20260531e';
import { render, refreshModal } from './render.js?v=20260531e';
  ...
  if(data && !data.error){
    ...
    if(data.clients && Array.isArray(data.clients) && data.clients.length>0){
      ...
      for(const c of state.clients){
        if(!c.calendlyUrl){
          const { getClientConfig } = await import('./client-info.js?v=20260531e');
    ...
    if(data.appointments && Array.isArray(data.appointments)){
      const { getToday } = await import('./utils.js?v=20260531e');
    ...
    const { runServiceAreaChecks } = await import('./maps.js?v=20260531e');
    ...
    if((isAdmin()||isEmployee()) && !state.archiveLoaded){
      const { loadArchive } = await import('./archive.js?v=20260531e');
      loadArchive(true);
    }
  } else {
    if(state.deals.length===0){
      const { getTestData } = await import('./config.js?v=20260531e');
...
export async function initialSync(isStartup) {
  ...
  try {
    ...
    render();
    if (isStartup) {
      import('./dashboard.js?v=20260531e').then(m => m.clearDashboardArchiveCache && m.clearDashboardArchiveCache()).catch(() => {});
    }
    ...
    if (savedSettings && Object.keys(savedSettings).length > 0) {
      const { applySettings } = await import('./settings.js?v=20260531e');
    ...
    if (!_cachedIsAdmin) {
      const { isAdmin: _isAdmin } = await import('./auth.js?v=20260531e');
    ...
    if (isStartup) {
      import('./activities.js?v=20260531e').then(m => m.replayPendingActivities && m.replayPendingActivities()).catch(() => {});
    }
