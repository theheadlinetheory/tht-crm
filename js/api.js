...
// ═══════════════════════════════════════════════════════════
import { API_URL, SUPABASE_URL, SUPABASE_ANON_KEY } from './config.js?v=20260908145001';
import { supabase } from './supabase-client.js?v=20260908145001';
export { supabase };
import { state, store, pendingWrites, failedWriteQueue, pendingDealFields, deletedDealIds, deletedActivityIds, completedActivityIds, deletedClientIds, inFlightActivityIds } from './app.js?v=20260908145001';
...
export async function syncFromSheet(){
  ...
  if(data && !data.error){
    ...
    if(data.appointments && Array.isArray(data.appointments)){
      const { getToday } = await import('./utils.js?v=20260908145001');
    ...
    const { runServiceAreaChecks } = await import('./maps.js?v=20260908145001');
    ...
    if((isAdmin()||isEmployee()) && !state.archiveLoaded){
      const { loadArchive } = await import('./archive.js?v=20260908145001');
      loadArchive(true);
    }
  } else {
    if(state.deals.length===0){
      const { getTestData } = await import('./config.js?v=20260908145001');
...
const FIELD_MAP = {
  ...
  setup_fee_deposit: 'setupFeeDeposit',
  setup_fee_spread_count: 'setupFeeSpreadCount',
  setup_fee_leads_billed: 'setupFeeLeadsBilled',
  ...
  service_area_polygons: 'serviceAreaPolygons',
  ...
  booking_sms_sent_at: 'bookingSmsSentAt',
  home_base: 'homeBase',
  ...
  date_passed: 'datePassed', // pass_offs — without this the Date column and the date filter see undefined
...
export async function initialSync(isStartup) {
  ...
  try {
    ...
    if (isStartup) {
      import('./dashboard.js?v=20260908145001').then(m => m.clearDashboardArchiveCache && m.clearDashboardArchiveCache()).catch(() => {});
    }
    ...
    if (savedSettings && Object.keys(savedSettings).length > 0) {
      const { applySettings } = await import('./settings.js?v=20260908145001');
    ...
    if (!_cachedIsAdmin) {
      const { isAdmin: _isAdmin } = await import('./auth.js?v=20260908145001');
    ...
    state.passOffs = (passOffs || []).map(normalizeRow);
    state.passOffsLoaded = true;
    state.retargetHistory = (retargetHistory || []).map(normalizeRow);
    state.retargetExports = (retargetExports || []).map(normalizeRow);
    state.retargetLoaded = true;
    ...
    // Prune activity caches — remove IDs for activities no longer in server data
    const fetchedActIds = new Set(freshActivities.map(a => String(a.id)));
    if (deletedActivityIds.size) {
      for (const id of deletedActivityIds) {
        if (!fetchedActIds.has(id)) deletedActivityIds.delete(id);
      }
      localStorage.setItem('tht_deletedActs', JSON.stringify([...deletedActivityIds]));
    }
    if (completedActivityIds.size) {
      for (const id of completedActivityIds) {
        if (!fetchedActIds.has(id)) completedActivityIds.delete(id);
      }
      localStorage.setItem('tht_completedActs', JSON.stringify([...completedActivityIds]));
    }
    ...
    if (isStartup) {
      import('./activities.js?v=20260908145001').then(m => m.replayPendingActivities && m.replayPendingActivities()).catch(() => {});
    }
    ...
    state.synced = true;
    state.syncing = false;
    render(); // always surface a load failure, even on the weekly tab
  }
}
...
function scheduleResubscribe() {
  ...
  _resubscribeTimer = setTimeout(async () => {
    ...
    console.warn('[Realtime] Reconnecting channels...');
...
export async function subscribeRealtime() {
  ...
  const rid = ++_realtimeId;
  supabase.channel(`deals-changes-${rid}`)
    ...
    .subscribe((status) => {
      if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') scheduleResubscribe();
    });

  supabase.channel(`activities-changes-${rid}`)
  ...
  supabase.channel(`clients-changes-${rid}`)
  ...
  supabase.channel(`appointments-changes-${rid}`)
  ...
  supabase.channel(`tracker-changes-${rid}`)
  ...
  supabase.channel(`demo-tracker-changes-${rid}`)
  ...
  supabase.channel(`pass-offs-changes-${rid}`)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'pass_offs' }, payload => {
      applyRealtimeEvent('pass_offs', payload);
      debouncedRealtimeRender();
    })
    .subscribe();
}
...
function applyRealtimeEvent(table, payload) {
  ...
  const stateKey = table === 'rerun_queue' ? 'rerunQueue' : table === 'lead_tracker' ? 'trackerEntries' : table === 'demo_tracker' ? 'demoEntries' : table === 'pass_offs' ? 'passOffs' : table;
...
const DEALS_LIGHT_COLS = 'id,company,contact,email,phone,value,stage,pipeline,flag,notes,sl_lead_id,sl_campaign_id,campaign_name,lead_category,created_at,updated_at,website,location,smartlead_url,forwarded_at,mobile_phone,pushed_to_tracker,pushed_to_ghl,address,client_stage,booked_date,booked_time,booked_for,booked_timezone,cal_name,cal_email,cal_notes,has_new_reply,has_new_text,owner_override,job_title,linkedin_url,passoff_instructions,passoff_sent_at,suggested_updates,contact2,contact3,phone2,phone3,title2,title3,email2,email3,email4,reply_snippet,retarget_campaign,retarget_date,retarget_status,retarget_count,auto_followup_started_at,booking_sms_sent_at,vertical';
...
export const sbCreateActivity = (fields) => sbCall(async () => {
  const { error } = await supabase.from('activities').upsert(fields, { onConflict: 'id' });
  if (error) throw error;
...
export const sbUpdateInteraction = (id, fields) => sbCall(async () => {
  const { error } = await supabase.from('interactions').update(fields).eq('id', id);
  if (error) throw error;
...
// ─── Pass-Offs CRUD ───
export const sbGetPassOffs = () => sbCall(async () => {
  const { data, error } = await supabase.from('pass_offs').select('*').order('date_passed', { ascending: false });
  if (error) throw error;
  return data;
}, { label: 'Load pass-offs' });

export const sbUpdatePassOff = (id, fields) => sbCall(async () => {
  const { error } = await supabase.from('pass_offs').update(fields).eq('id', id);
  if (error) throw error;
}, { label: 'Update pass-off' });

export const sbDeletePassOff = (id) => sbCall(async () => {
  const { error } = await supabase.from('pass_offs').delete().eq('id', id);
  if (error) throw error;
}, { label: 'Delete pass-off' });
...
// so this is the only place the second/third contacts, mobile and secondary
// numbers, extra emails, LinkedIn and street address still exist.
export const sbGetWonAcquisitionCards = () => sbCall(async () => {
  const PAGE = 1000;
  let all = [];
  for (let off = 0; ; off += PAGE) {
    const { data, error } = await supabase.from('archive')
      .select('id,original_data').eq('archive_status', 'Closed Won').range(off, off + PAGE - 1);
    if (error) throw error;
    all = all.concat(data || []);
    if (!data || data.length < PAGE) break;
  }
  return all;
}, { label: 'Load acquisition cards' });
...
export const sbGetRetargetHistory = () => sbCall(async () => {
  const { data, error } = await supabase.from('retarget_history').select('*').order('exported_at', { ascending: false });
  if (error) throw error;
  return data;
}, { label: 'Load retarget history' });

export const sbGetRetargetExports = () => sbCall(async () => {
  const { data, error } = await supabase.from('retarget_exports').select('*').order('exported_at', { ascending: false });
  if (error) throw error;
  return data;
}, { label: 'Load retarget exports' });

export const sbCreateRetargetHistory = (fields) => sbCall(async () => {
  const { data, error } = await supabase.from('retarget_history').insert(fields).select().single();
  if (error) throw error;
  return data;
}, { label: 'Create retarget history' });

export const sbCreateRetargetExport = (fields) => sbCall(async () => {
  const { data, error } = await supabase.from('retarget_exports').insert(fields).select().single();
  if (error) throw error;
  return data;
}, { label: 'Create retarget export' });

export const sbUpdateRetargetExport = (id, fields) => sbCall(async () => {
  const { error } = await supabase.from('retarget_exports').update(fields).eq('id', id);
  if (error) throw error;
}, { label: 'Update retarget export' });

export const sbUpdateRetargetHistory = (id, fields) => sbCall(async () => {
  const { error } = await supabase.from('retarget_history').update(fields).eq('id', id);
  if (error) throw error;
}, { label: 'Update retarget history' });

export const sbBatchUpdateDeals = (ids, fields) => sbCall(async () => {
  const { error } = await supabase.from('deals').update(fields).in('id', ids);
  if (error) throw error;
