...
// ═══════════════════════════════════════════════════════════
import { API_URL, SUPABASE_URL, SUPABASE_ANON_KEY } from './config.js?v=20260908143234';
import { supabase } from './supabase-client.js?v=20260908143234';
export { supabase };
import { state, store, pendingWrites, failedWriteQueue, pendingDealFields, deletedDealIds, deletedActivityIds, completedActivityIds, deletedClientIds, inFlightActivityIds } from './app.js?v=20260908143234';
...
export async function syncFromSheet(){
  ...
  if(data && !data.error){
    ...
    if(data.appointments && Array.isArray(data.appointments)){
      const { getToday } = await import('./utils.js?v=20260908143234');
    ...
    const { runServiceAreaChecks } = await import('./maps.js?v=20260908143234');
    ...
    if((isAdmin()||isEmployee()) && !state.archiveLoaded){
      const { loadArchive } = await import('./archive.js?v=20260908143234');
      loadArchive(true);
    }
  } else {
    if(state.deals.length===0){
      const { getTestData } = await import('./config.js?v=20260908143234');
...
export async function initialSync(isStartup) {
  ...
  try {
    ...
    if (isStartup) {
      import('./dashboard.js?v=20260908143234').then(m => m.clearDashboardArchiveCache && m.clearDashboardArchiveCache()).catch(() => {});
    }
    ...
    if (savedSettings && Object.keys(savedSettings).length > 0) {
      const { applySettings } = await import('./settings.js?v=20260908143234');
    ...
    if (!_cachedIsAdmin) {
      const { isAdmin: _isAdmin } = await import('./auth.js?v=20260908143234');
    ...
    if (isStartup) {
      import('./activities.js?v=20260908143234').then(m => m.replayPendingActivities && m.replayPendingActivities()).catch(() => {});
    }
