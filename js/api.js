...
// ═══════════════════════════════════════════════════════════
import { API_URL } from './config.js?v=20260715d';
import { supabase } from './supabase-client.js?v=20260715d';
export { supabase };
import { state, store, pendingWrites, failedWriteQueue, pendingDealFields, deletedDealIds, deletedActivityIds, completedActivityIds, deletedClientIds, inFlightActivityIds } from './app.js?v=20260715d';
import { render, refreshModal } from './render.js?v=20260715d';
...
export async function syncFromSheet(){
  ...
  if(data && !data.error){
    ...
    if(data.appointments && Array.isArray(data.appointments)){
      const { getToday } = await import('./utils.js?v=20260715d');
    ...
    const { runServiceAreaChecks } = await import('./maps.js?v=20260715d');
    ...
    if((isAdmin()||isEmployee()) && !state.archiveLoaded){
      const { loadArchive } = await import('./archive.js?v=20260715d');
      loadArchive(true);
    }
  } else {
    if(state.deals.length===0){
      const { getTestData } = await import('./config.js?v=20260715d');
...
export async function initialSync(isStartup) {
  ...
  try {
    ...
    if (isStartup) {
      import('./dashboard.js?v=20260715d').then(m => m.clearDashboardArchiveCache && m.clearDashboardArchiveCache()).catch(() => {});
    }
    ...
    if (!_cachedIsAdmin) {
      const { isAdmin: _isAdmin } = await import('./auth.js?v=20260715d');
    ...
    if (isStartup) {
      import('./activities.js?v=20260715d').then(m => m.replayPendingActivities && m.replayPendingActivities()).catch(() => {});
    }
