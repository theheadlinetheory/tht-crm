...
// ═══════════════════════════════════════════════════════════
import { API_URL, SUPABASE_URL, SUPABASE_ANON_KEY } from './config.js?v=20260711b';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
export { supabase };
import { state, store, pendingWrites, failedWriteQueue, pendingDealFields, deletedDealIds, deletedActivityIds, completedActivityIds, deletedClientIds, inFlightActivityIds } from './app.js?v=20260711b';
  ...
  if(data && !data.error){
    ...
    if(data.clients && Array.isArray(data.clients) && data.clients.length>0){
      ...
      for(const c of state.clients){
        if(!c.calendlyUrl){
          const { getClientConfig } = await import('./client-info.js?v=20260711b');
    ...
    if(data.appointments && Array.isArray(data.appointments)){
      const { getToday } = await import('./utils.js?v=20260711b');
    ...
    const { runServiceAreaChecks } = await import('./maps.js?v=20260711b');
    ...
    if((isAdmin()||isEmployee()) && !state.archiveLoaded){
      const { loadArchive } = await import('./archive.js?v=20260711b');
      loadArchive(true);
    }
  } else {
    if(state.deals.length===0){
      const { getTestData } = await import('./config.js?v=20260711b');
...
export async function initialSync(isStartup) {
  ...
  try {
    ...
    render();
    if (isStartup) {
      import('./dashboard.js?v=20260711b').then(m => m.clearDashboardArchiveCache && m.clearDashboardArchiveCache()).catch(() => {});
    }
    ...
    if (savedSettings && Object.keys(savedSettings).length > 0) {
      const { applySettings } = await import('./settings.js?v=20260711b');
    ...
    if (!_cachedIsAdmin) {
      const { isAdmin: _isAdmin } = await import('./auth.js?v=20260711b');
    ...
    if (isStartup) {
      import('./activities.js?v=20260711b').then(m => m.replayPendingActivities && m.replayPendingActivities()).catch(() => {});
    }
