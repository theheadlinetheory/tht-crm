...
import { invokeEdgeFunction } from './api.js?v=20260720203041';
import { esc, str, svgIcon, getToday, isValidDate, fmtDate, fmtTime12, uid } from './utils.js?v=20260720203041';
import { findClientForDeal, lookupClientInfo, getWarmCallQA } from './client-info.js?v=20260720203041';
import { isAdmin, isEmployee } from './auth.js?v=20260720203041';
...
export function initWcCalendlyInline(baseUrl, clientName, dealId){
  ...
  import('./calendly.js?v=20260720203041').then(mod=>mod.setCalendlyBookingDealId(dealId));
...
export async function pushToClientInfo(dealId){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { CLIENT_INFO_SHEET_ID } = await import('./config.js?v=20260720203041');
...
export async function doCopyLeadInfo(dealId){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { buildLeadMessage } = await import('./email.js?v=20260720203041');
  const client=findClientForDeal(deal)||state.clients.find(c=>c.name===deal.stage);
...
function initStreetView() {
  ...
  if (!address) {
    container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#9ca3af;font-size:13px">No address available</div>';
    return;
  }

  import('./config.js?v=20260720203041').then(({ GOOGLE_MAPS_API_KEY }) => {
    ...
    loadGoogleMapsApi(GOOGLE_MAPS_API_KEY).then(() => {
      // Use Google's geocoder — handles highway names, suite numbers, etc. far better than Nominatim
      ...
      geocoder.geocode({ address }, (results, status) => {
        ...
        sv.getPanorama({ location: loc, radius: 200 }, (data, svStatus) => {
          if (svStatus === google.maps.StreetViewStatus.OK) {
            container.innerHTML = '';
            new google.maps.StreetViewPanorama(container, {
              position: data.location.latLng,
              pov: { heading: google.maps.geometry.spherical.computeHeading(data.location.latLng, loc), pitch: 0 },
              zoom: 1,
