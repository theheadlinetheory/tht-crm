...
import { invokeEdgeFunction } from './api.js?v=20260910165154';
import { esc, str, svgIcon, getToday, isValidDate, fmtDate, fmtTime12, uid } from './utils.js?v=20260910165154';
import { findClientForDeal, lookupClientInfo, getWarmCallQA } from './client-info.js?v=20260910165154';
import { isAdmin, isEmployee } from './auth.js?v=20260910165154';
...
export function initWcCalendlyInline(baseUrl, clientName, dealId){
  ...
  import('./calendly.js?v=20260910165154').then(mod=>mod.setCalendlyBookingDealId(dealId));
...
export async function pushToClientInfo(dealId){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { CLIENT_INFO_SHEET_ID } = await import('./config.js?v=20260910165154');
...
export async function doCopyLeadInfo(dealId){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { buildLeadMessage } = await import('./email.js?v=20260910165154');
  const client=findClientForDeal(deal)||state.clients.find(c=>c.name===deal.stage);
...
function initStreetView() {
  ...
  import('./config.js?v=20260910165154').then(({ GOOGLE_MAPS_API_KEY }) => {
