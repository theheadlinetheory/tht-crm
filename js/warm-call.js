...
import { invokeEdgeFunction } from './api.js?v=20260803145856';
import { esc, str, svgIcon, getToday, isValidDate, fmtDate, fmtTime12, uid } from './utils.js?v=20260803145856';
import { findClientForDeal, lookupClientInfo, getWarmCallQA } from './client-info.js?v=20260803145856';
...
export function initWcCalendlyInline(baseUrl, clientName, dealId){
  ...
  import('./calendly.js?v=20260803145856').then(mod=>mod.setCalendlyBookingDealId(dealId));
...
export async function pushToClientInfo(dealId){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { CLIENT_INFO_SHEET_ID } = await import('./config.js?v=20260803145856');
...
function initStreetView() {
  ...
  import('./config.js?v=20260803145856').then(({ GOOGLE_MAPS_API_KEY }) => {
