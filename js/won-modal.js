...
// ═══════════════════════════════════════════════════════════
import { state } from './app.js?v=20260918152403';
import { str, esc, getToday } from './utils.js?v=20260918152403';
import { createClientRecord, deriveTimezone } from './client-info.js?v=20260918152403';
import { ensureLeadTrackerSheet } from './lead-tracker-sheet.js?v=20260918152403';
import { invokeEdgeFunction, showToast } from './api.js?v=20260918152403';
import { isAdmin } from './auth.js?v=20260918152403';
import { prepaidNote } from './retainer-billing.js?v=20260918152403';
import { createSmartleadPortal } from './smartlead-portal.js?v=20260918152403';
import { PAYMENT_CADENCES, TERM_UNITS } from './client-terms.js?v=20260918152403';
  ...
  if (type === 'retainer') {
    el.innerHTML = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      ...
      <div>${lbl('Initial term (blank = open-ended)')}<div style="display:flex;gap:6px">
        ...
        <select id="won-term-unit" style="flex:1;min-width:0;box-sizing:border-box;padding:7px 10px;border:1px solid #cbd5e1;border-radius:6px;font-size:13px;margin-top:3px">${TERM_UNITS.map((u) => `<option value="${u}">${u}</option>`).join('')}</select>
      </div></div>
...
function buildFields() {
  ...
  const f = {
    ...
    status: 'active',
    activatedDate: closedDate, // createClientRecord spreads every field into the clients insert — nothing else goes in here
    billingModel: type,
  };
  if (type === 'retainer') {
  ...
  if (_w) _w.closedDate = closedDate; // for the Closed Won mark (runSteps), kept off `f` so it never reaches the clients row
  return f;
}
...
async function runSteps(f, startIdx) {
  try {
    ...
    if (startIdx <= 1 && !_w.archived) {
      ...
      await deleteDeal(_w.deal.id, 'Closed Won', f.name, { asOf: _w.closedDate || getToday() });
      _w.archived = true;
    }
