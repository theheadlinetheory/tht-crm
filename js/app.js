...
// ═══════════════════════════════════════════════════════════
import { REPLY_CHECK_INTERVAL, REPLY_BACKEND_POLL_INTERVAL, SYNC_INTERVAL, DEFAULT_CLIENT_STAGES } from './config.js?v=20260527a';
import { render } from './render.js?v=20260527a';
import { syncFromSheet, pollReplyStatus, triggerBackendReplyCheck, initialSync, subscribeRealtime, flushRealtimeQueue } from './api.js?v=20260527a';
import { isAdmin, isClient, isEmployee, currentUser, loadCampaignAssignments, listenCampaignAssignments, setupAuthListener, db } from './auth.js?v=20260527a';
import { initJustCallDialer } from './dialer.js?v=20260527a';
import { esc, svgIcon } from './utils.js?v=20260527a';
import './email.js?v=20260527a';
import './blooio.js?v=20260527a';
...
export async function loadClientArchive(){
  if(!isClient()||!currentUser.clientName) return;
  const { sbGetArchive, normalizeRow } = await import('./api.js?v=20260527a');
...
export async function restoreDeal(dealId){
  ...
  if(isSheetArchived){
    const { sbRestoreFromArchive, initialSync } = await import('./api.js?v=20260527a');
    pendingWrites.value++;
    try {
...
export async function initApp(){
  ...
  try {
    ...
    try {
      const { loadClientConfig } = await import('./client-info.js?v=20260527a');
    ...
    if(window.SERVICE_AREA_POLYGONS){
      try {
        const { setServiceAreaData } = await import('./maps.js?v=20260527a');
    ...
    try {
      ...
      if (urlDealId) {
        const { openDeal } = await import('./deal-modal.js?v=20260527a');
    ...
    // if(!isClient()) import('./number-health.js?v=20260527a').then(m => m.loadNumberHealth()).catch(e => console.warn('Number health load failed:', e));
    if(!isClient()) import('./warm-call.js?v=20260527a').catch(e => console.warn('Warm call module load failed:', e));
    // Load nurture data for Due Today banner
    if(!isClient()){
      import('./rerun.js?v=20260527a').then(m => m.loadNurtureData()).catch(e => console.warn('Nurture data load failed:', e));
    }
  } catch(e) {
...
// ─── Event Delegation ───
import { initDelegation } from './delegate.js?v=20260527a';
initDelegation();
