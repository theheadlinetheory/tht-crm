...
// ═══════════════════════════════════════════════════════════
import { REPLY_CHECK_INTERVAL, REPLY_BACKEND_POLL_INTERVAL, SYNC_INTERVAL } from './config.js?v=20260905075300';
import { render } from './render.js?v=20260905075300';
import { syncFromSheet, pollReplyStatus, triggerBackendReplyCheck, initialSync, subscribeRealtime, flushRealtimeQueue } from './api.js?v=20260905075300';
import { isAdmin, isEmployee, loadCampaignAssignments, listenCampaignAssignments, setupAuthListener } from './auth.js?v=20260905075300';
import { initJustCallDialer } from './dialer.js?v=20260905075300';
import './email.js?v=20260905075300';
import './blooio.js?v=20260905075300';

// ─── Local import for vars used in this file ───
import { state } from './state.js?v=20260905075300';
...
} from './state.js?v=20260905075300';
...
export async function initApp(){
  ...
  try {
    ...
    if(isAdmin()||isEmployee()){
      ...
      const { loadRoutingRules, listenRoutingRules } = await import('./routing-rules.js?v=20260905075300');
    ...
    if(window.SERVICE_AREA_POLYGONS){
      try {
        const { setServiceAreaData } = await import('./maps.js?v=20260905075300');
    ...
    try {
      ...
      if (urlDealId) {
        const { openDeal } = await import('./deal-modal.js?v=20260905075300');
    ...
    initJustCallDialer();
    import('./number-health.js?v=20260905075300').then(m => m.loadNumberHealth()).catch(e => console.warn('Number health load failed:', e));
    import('./warm-call.js?v=20260905075300').catch(e => console.warn('Warm call module load failed:', e));
    // Load nurture data for Due Today banner
    import('./rerun.js?v=20260905075300').then(m => m.loadNurtureData()).catch(e => console.warn('Nurture data load failed:', e));
  } catch(e) {
...
// ─── Event Delegation ───
import { initDelegation } from './delegate.js?v=20260905075300';
initDelegation();
