...
import { state, pendingWrites, pendingDealFields } from './app.js?v=20260905073617';
import { flushRealtimeQueue } from './api.js?v=20260905073617';
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, REACTIVATION_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, SUPABASE_URL, SUPABASE_ANON_KEY, detectCountry, ACQ_STRATEGY_BOOKERS, ACQ_DEMO_BOOKERS } from './config.js?v=20260905073617';
import { render, refreshModal } from './render.js?v=20260905073617';
import { apiGet, invokeEdgeFunction, sbUpdateDeal, sbGetDealHeavyFields, camelToSnake } from './api.js?v=20260905073617';
import { esc, str, getToday, TODAY, uid, svgIcon, fmtDate, fmtTime12, fmtTimestamp, stripHtml, applyTemplate } from './utils.js?v=20260905073617';
import { DEFAULT_INSTRUCTIONS_TEMPLATE } from './settings.js?v=20260905073617';
import { renderBookingSmsButton } from './booking-sms.js?v=20260905073617';
import { JUSTCALL_DISPOSITIONS, applyDisposition, isCallTouchpoint } from './call-touchpoints.js?v=20260905073617';
import { HELD, NO_SHOW, OUTCOME_PREFIX, DEMO_OUTCOME_PREFIX, markDisco, markDemo, outcomeSelect, demoOutcomeSelect } from './disco-outcome.js?v=20260905073617';
import { renderDealInvoiceButton } from './deal-invoice.js?v=20260905073617';
import { isAdmin, isEmployee, loadAssignableUsers } from './auth.js?v=20260905073617';
import { saveDeal, createDeal, moveDeal, deleteDeal as deleteDealFn } from './deals.js?v=20260905073617';
import { showAcquisitionRemovalPicker } from './removal-reason.js?v=20260905073617';
import { addActivity, assignSequence, getSopDays, renderUpcomingMeetings, generateAppointmentSequence, reschedulePreCallSequence, assignNoShowSequence } from './activities.js?v=20260905073617';
import { addClient, findClientForDeal, lookupClientInfo, isRetainerClient, getWarmCallQA } from './client-info.js?v=20260905073617';
import { getStagesForPipeline } from './dashboard.js?v=20260905073617';
import { renderServiceAreaMap, findPolygonForClient, serviceAreaResults, geocodeCache, geocodeAndCheckDeal } from './maps.js?v=20260905073617';
import { loadSmartleadThread, renderSmartleadThread, renderThreadMessage, toggleFullThread, getThreadCache, openSendToClientPreview, doSendToClientThread } from './threads.js?v=20260905073617';
import { renderPassoffSection, startTranscriptPolling, stopTranscriptPolling } from './passoff.js?v=20260905073617';
import './blooio.js';
import './google-task.js?v=20260905073617';
import './demo-tracker.js';
import { renderDealRetargetHistory } from './retargeting.js?v=20260905073617';
...
export async function doWonDrop(){
  ...
  if(deal.pipeline==='Acquisition'){
    ...
    const { openWonModal } = await import('./won-modal.js?v=20260905073617');
    openWonModal(deal);
    return;
  }
  ...
  try {
    const { autoPushToTracker } = await import('./email.js?v=20260905073617');
    await autoPushToTracker(deal);
    wonSuccess = true;
  } catch(e){
    console.error('Won drop action failed:', e);
    const { showToast } = await import('./api.js?v=20260905073617');
    showToast('Won action failed: ' + e.message, 'error');
  }

  if(wonSuccess) {
    const { deleteDeal } = await import('./deals.js?v=20260905073617');
...
async function loadInteractions(dealId) {
  try {
    const { sbGetInteractions } = await import('./api.js?v=20260905073617');
...
// Demos before the level 05 baseline were settled by the 2026-08-27 re-QC,
...
async function attachRecording(interactionId, dealId) {
  ...
  if (!url) return;
  const { sbUpdateInteraction, showToast } = await import('./api.js?v=20260905073617');
...
async function addInteraction(dealId) {
  ...
  const { sbCreateInteraction } = await import('./api.js?v=20260905073617');
...
async function deleteInteraction(id, dealId) {
  const { sbDeleteInteraction } = await import('./api.js?v=20260905073617');
...
async function editInteractionDate(id, dealId) {
  ...
  const { sbUpdateInteraction } = await import('./api.js?v=20260905073617');
...
window.markAtYourConvenience = async (dealId) => {
  ...
  const { showToast: toast } = await import('./api.js?v=20260905073617');
...
export function confirmScheduleAndCopy(){
  ...
  const client=findClientForDeal(deal)||state.clients.find(c=>c.name===deal.stage);
  if(client && dateVal){
    import('./calendly.js?v=20260905073617').then(mod=>{
...
window.pushToClientSheet = async function(dealId) {
  ...
  try {
    ...
    btn.disabled = false;
    const { showToast: toast2 } = await import('./api.js?v=20260905073617');
