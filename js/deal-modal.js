...
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, TRACKER_ZERO_COST_CLIENTS } from './config.js';
...
export function doAddActivity(dealId){
  const typeEl=document.getElementById('add-act-type');
  const subEl=document.getElementById('add-act-subject');
  const dateEl=document.getElementById('add-act-date');
...
export function renderDealModal(deal){
  ...
  if((isAdmin()||isEmployee()) && str(deal.slLeadId).trim() && str(deal.slCampaignId).trim()){
    ...
    <div id="sl-thread-${esc(deal.id)}"></div>`;
  }
...
`;
      ...
      if(isOn('enableTracker')){
        ...
        const isZeroCostClient=TRACKER_ZERO_COST_CLIENTS.some(ex=>(matchedClient.name||'').toLowerCase().includes(ex));
        h+=`<div style="margin:0 0 8px 0">
...
export async function openSendToClientPreview(dealId, clientName){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { getClientThreadId, CLIENT_THREAD_IDS, lookupClientInfo } = await import('./client-info.js');
