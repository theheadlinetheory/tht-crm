...
import { state, pendingWrites, pendingDealFields } from './app.js?v=20260919033445';
import { flushRealtimeQueue, showToast } from './api.js?v=20260919033445';
import { ACQUISITION_STAGES, NURTURE_STAGES, DEAL_VERTICALS, SOP_DAYS, REACTIVATION_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, SUPABASE_URL, SUPABASE_ANON_KEY, detectCountry, ACQ_STRATEGY_BOOKERS, ACQ_DEMO_BOOKERS } from './config.js?v=20260919033445';
import { render, refreshModal } from './render.js?v=20260919033445';
import { apiGet, invokeEdgeFunction, sbUpdateDeal, sbGetDealHeavyFields, camelToSnake } from './api.js?v=20260919033445';
import { esc, str, getToday, TODAY, uid, svgIcon, brandIcon, fmtDate, fmtTime12, fmtTimestamp, stripHtml, applyTemplate } from './utils.js?v=20260919033445';
import { DEFAULT_INSTRUCTIONS_TEMPLATE } from './settings.js?v=20260919033445';
import { renderBookingSmsButton } from './booking-sms.js?v=20260919033445';
import { JUSTCALL_DISPOSITIONS, applyDisposition, isCallTouchpoint } from './call-touchpoints.js?v=20260919033445';
import { HELD, NO_SHOW, OUTCOME_PREFIX, DEMO_OUTCOME_PREFIX, markDisco, markDemo, outcomeSelect, demoOutcomeSelect } from './disco-outcome.js?v=20260919033445';
import { renderDealInvoiceButton } from './deal-invoice.js?v=20260919033445';
import { isAdmin, isEmployee, loadAssignableUsers } from './auth.js?v=20260919033445';
import { saveDeal, createDeal, moveDeal, deleteDeal as deleteDealFn } from './deals.js?v=20260919033445';
import { showAcquisitionRemovalPicker } from './removal-reason.js?v=20260919033445';
import { addActivity, assignSequence, getSopDays, renderUpcomingMeetings, generateAppointmentSequence, reschedulePreCallSequence, assignNoShowSequence } from './activities.js?v=20260919033445';
import { addClient, findClientForDeal, lookupClientInfo, isRetainerClient, getWarmCallQA } from './client-info.js?v=20260919033445';
import { getStagesForPipeline } from './dashboard.js?v=20260919033445';
import { renderServiceAreaMap, findPolygonForClient, serviceAreaResults, geocodeCache, geocodeAndCheckDeal } from './maps.js?v=20260919033445';
import { loadSmartleadThread, renderSmartleadThread, renderThreadMessage, toggleFullThread, getThreadCache, openSendToClientPreview, doSendToClientThread } from './threads.js?v=20260919033445';
import { renderGmailSection } from './gmail-threads.js?v=20260919033445';
import { renderPassoffSection, startTranscriptPolling, stopTranscriptPolling } from './passoff.js?v=20260919033445';
import './blooio.js?v=20260919033445';
import './google-task.js?v=20260919033445';
import './demo-tracker.js?v=20260919033445';
import { renderDealRetargetHistory } from './retargeting.js?v=20260919033445';
...
export function closeDealModal(){
  ...
  // An archived deal opened to look at (archive.js) is parked in state.deals only while its card is open.
  if(state.selectedDeal && state.selectedDeal._archived){ const id=String(state.selectedDeal.id); state.deals=state.deals.filter(d=>String(d.id)!==id); }
...
// Shared acquisition call picker — routes a deal to a chosen person's Calendly.
// options: [{label, color, type}] where type is a key in ACQ_CALENDLY_URLS.
function showAcqCallPicker(dealId, title, options) {
  const existing = document.getElementById('acq-call-picker');
  if (existing) existing.remove();
  const div = document.createElement('div');
  div.id = 'acq-call-picker';
  ...
  const buttons = options.map(o =>
    `<button class="btn btn-primary" style="width:100%;padding:12px;font-size:13px;background:${o.color};border-color:${o.color}" onclick="document.getElementById('acq-call-picker').remove();openAcqCalendly('${dealId}','${o.type}')">${o.label}</button>`
  ).join('');
  div.innerHTML = `<div style="background:#fff;border-radius:12px;padding:24px;width:320px;box-shadow:0 8px 30px rgba(0,0,0,.2)">
    ...
    <button class="btn btn-ghost" style="width:100%;margin-top:12px;font-size:12px" onclick="document.getElementById('acq-call-picker').remove()">Cancel</button>
  </div>`;
  document.body.appendChild(div);
}

window.showStrategyCallPicker = (dealId) => showAcqCallPicker(dealId, "Who's booking the strategy call?", ACQ_STRATEGY_BOOKERS);

window.showDemoCallPicker = (dealId) => showAcqCallPicker(dealId, "Who's taking the demo call?", ACQ_DEMO_BOOKERS);
...
window.showArchiveReasonPicker = function(dealId){
  ...
  // Acquisition leads get the sales-pipeline reason list (removal-reason.js):
  // the reason decides level 02's denominator, so it cannot be skipped.
  if (isAcq) {
    showAcquisitionRemovalPicker([dealId], {
      onPick: (label) => deleteDealFn(dealId, label),
      onNurture: () => window.triggerNurtureFromArchive(dealId),
    });
    return;
  }
  const div = document.createElement('div');
  ...
  div.innerHTML = `<div style="background:#fff;border-radius:12px;padding:24px;width:340px;box-shadow:0 8px 30px rgba(0,0,0,.2)">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${isAcq ? acqButtons : ''}
      ${!isAcq?'<button class="btn" style="width:100%;justify-content:start;padding:10px 14px;background:#fffbeb;color:#d97706;border:1px solid #fde68a" onclick="document.getElementById(\'archive-reason-picker\').remove();deleteDeal(\''+dealId+'\',\'Passed Off\')">Passed Off</button>':''}
      ...
      ${!isAcq?'<button class="btn" style="width:100%;justify-content:start;padding:10px 14px;background:#fef2f2;color:#dc2626;border:1px solid #fecaca" onclick="document.getElementById(\'archive-reason-picker\').remove();deleteDeal(\''+dealId+'\',\'Not in Service Area\')">Not in Service Area</button>':''}
...
window.updateDealVertical = function(dealId, val) {
  const deal = state.deals.find(d => String(d.id) === String(dealId));
  if (!deal) return;
  deal.vertical = val;
  pendingWrites.value++;
  sbUpdateDeal(dealId, { vertical: val }).catch(e => console.error('Update vertical failed:', e)).finally(() => { pendingWrites.value--; });
};
...
// An archived deal's card is for looking (archive.js): every write path refuses and points at the Funnel row, where the
// answer is dated to the call or demo it belongs to (hardening review, 2026-09-15).
function archivedReadOnly(){
  const d=state.selectedDeal;
  if(d && d._archived){ showToast('This deal is archived — record the outcome from its row on the Funnel tab','error'); return true; }
  return false;
}
export function updateDealField(key,val){
  if(archivedReadOnly()) return;
  if(!state.selectedDeal) return;
...
export function doAssignSequenceWithDate(dealId,day,seq){
  const dateEl=document.getElementById('sop-target-date');
  const targetDate=dateEl?dateEl.value:getToday();
  const sopDaysMap=seq==='reactivation'?REACTIVATION_DAYS:getSopDaysForDeal(dealId);
  const acts=sopDaysMap[day];
  if(!acts) return;
...
export function doLostDrop(){
  ...
  // Dragging an acquisition lead to the delete zone still has to say why.
  if(deal && deal.pipeline === 'Acquisition'){
    showAcquisitionRemovalPicker([id], {
      onPick: (label) => deleteDeal(id, label, client?client.name:''),
      onNurture: () => window.triggerNurtureFromArchive(id),
    });
    return;
  }
  deleteDeal(id, 'Deleted/Lost', client?client.name:'');
}
...
export async function doWonDrop(){
  ...
  if(deal.pipeline==='Acquisition'){
    ...
    const { openWonModal } = await import('./won-modal.js?v=20260919033445');
    openWonModal(deal);
    return;
  }
  ...
  try {
    const { autoPushToTracker } = await import('./email.js?v=20260919033445');
    await autoPushToTracker(deal);
    wonSuccess = true;
  } catch(e){
    console.error('Won drop action failed:', e);
    const { showToast } = await import('./api.js?v=20260919033445');
    showToast('Won action failed: ' + e.message, 'error');
  }

  if(wonSuccess) {
    const { deleteDeal } = await import('./deals.js?v=20260919033445');
...
async function loadInteractions(dealId) {
  try {
    const { sbGetInteractions } = await import('./api.js?v=20260919033445');
    ...
    _interactionsCache[dealId] = (rows || []).map(r => ({
      ...
      createdAt: r.created_at, source: r.source || 'manual'
    }));
  } catch (e) {
...
// Timeline content is plain text, but recorded touchpoints append the recording
// URL on its own line — JustCall's for calls, Fathom's for meetings. Escape
// first, then linkify — never the other way round, or the anchor gets escaped
// too.
function fmtInteractionContent(content) {
  return esc(String(content || ''))
    .replace(/https?:\/\/[^\s<]+/g, (u) => {
      const label = /fathom\.video/i.test(u) ? 'Fathom recording'
        : /mail\.google\.com/i.test(u) ? 'Open in Gmail'
        : 'Recording';
      return '<a href="' + u + '" target="_blank" rel="noopener" style="color:#7c3aed;text-decoration:underline">' + label + '</a>';
    })
    .replace(/\n/g, '<br>');
}
...
//
// Normally nobody touches this. JustCall posts call.updated to the
// justcall-crm-touchpoint edge function the moment a disposition is set, and
// the timeline entry updates itself — see call-touchpoints.js.
//
// This is the correction path, and it only appears on a call that has NO
// disposition after all that: an old call from before the webhook existed, or
// one the rep never classified. Picking rewrites the entry's outcome in place
// rather than adding a second one.
const ALL_DISPOSITIONS = Object.entries(JUSTCALL_DISPOSITIONS)
  .flatMap(([group, values]) => values.map(v => group + ': ' + v));

// Discos before this date were settled by the level 02 audit, which read the
// call recordings lead by lead. Asking about one again would double-count it
// against a baseline that already contains it — and would put ~100 questions in
// front of a rep when the real backlog is a handful. Same cutoff the queue uses.
const DISCO_BASELINE_DATE = '2026-08-25';

/** True for a "Discovery call scheduled" entry whose meeting time has passed,
 *  is after the audited baseline, and has no outcome recorded on the deal. */
function needsDiscoOutcome(ev, allEvents) {
  const c = String(ev.content || '');
  if (!c.startsWith('Discovery call scheduled')) return false;
  const m = c.match(/for (\d{4}-\d{2}-\d{2} \d{2}:\d{2})/);
  if (!m) return false;
  if (m[1].slice(0, 10) <= DISCO_BASELINE_DATE) return false;
  if (new Date(m[1].replace(' ', 'T') + 'Z') > new Date()) return false;
  return !allEvents.some(x => {
    const s = String(x.content || '');
    // OUTCOME_PREFIX covers the four-way answers; HELD/NO_SHOW are the older
    // two-button ones, still honoured so an answered call is never re-asked.
    return s.startsWith(OUTCOME_PREFIX) || s.startsWith(HELD) || s.startsWith(NO_SHOW);
  });
}

// Demos before the demo → closed baseline were settled by the 2026-08-27 re-QC,
// which checked the client roster rather than the demo tracker. Re-asking would
// double-count against a baseline that already contains them.
const DEMO_BASELINE_DATE = '2026-08-20';

function needsDemoOutcome(ev, allEvents) {
  const c = String(ev.content || '');
  if (!c.startsWith('Demo scheduled')) return false;
  const m = c.match(/for (\d{4}-\d{2}-\d{2} \d{2}:\d{2})/);
  if (!m) return false;
  if (m[1].slice(0, 10) <= DEMO_BASELINE_DATE) return false;
  if (new Date(m[1].replace(' ', 'T') + 'Z') > new Date()) return false;
  // Keep asking while the latest answer is "Qualified — Pending" (showed, no
  // decision yet); stop once a final answer is on the Timeline.
  const marks = allEvents.filter(x => String(x.content || '').startsWith(DEMO_OUTCOME_PREFIX))
    .sort((a, b) => String(a.created_at || '').localeCompare(String(b.created_at || '')));
  if (!marks.length) return true;
  return /Qualified — Pending/.test(String(marks[marks.length - 1].content));
}

function renderDemoOutcomeButtons(dealId) {
  return `<div style="margin-top:6px;display:flex;align-items:center;gap:6px">
    <span style="font-size:11px;color:#6b7280">Demo outcome</span>
    <span onchange="markDemoFromTimeline('${esc(dealId)}',event.target.value)">${demoOutcomeSelect(dealId).replace(/onchange="[^"]*"/, '')}</span>
  </div>`;
}

async function markDemoFromTimeline(dealId, outcome) {
  if(archivedReadOnly()) return;
  if (!outcome) return;
  await markDemo(dealId, outcome);
  await loadInteractions(dealId);
  closeTimelinePanel();
  openTimelinePanel(dealId);
}

function renderDiscoOutcomeButtons(dealId) {
  return `<div style="margin-top:6px;display:flex;align-items:center;gap:6px">
    <span style="font-size:11px;color:#6b7280">Outcome</span>
    <span onchange="markDiscoFromTimeline('${esc(dealId)}',event.target.value)">${outcomeSelect(dealId).replace(/onchange="[^"]*"/, '')}</span>
  </div>`;
}

async function markDiscoFromTimeline(dealId, outcome) {
  if(archivedReadOnly()) return;
  if (!outcome) return;
  await markDisco(dealId, outcome);
  await loadInteractions(dealId);
  closeTimelinePanel();
  openTimelinePanel(dealId);
}
...
// ─── Meeting recordings (Fathom) ───
//
// A booked call already lands on the timeline as its own Meeting touchpoint, so
// the recording belongs ON that touchpoint — the same shape JustCall gives a
// call, where the recording URL sits on its own line under the call. That way
// the timeline is the one place every recorded interaction lives: JustCall for
// the phone calls, Fathom for the demos and discos.
const MEETING_PREFIXES = ['Demo scheduled', 'Discovery call scheduled'];

function isMeetingTouchpoint(content) {
  const c = String(content || '');
  return MEETING_PREFIXES.some(p => c.startsWith(p));
}

function hasRecording(content) {
  return /https?:\/\/\S+/i.test(String(content || ''));
}

function renderRecordingAttach(interactionId, dealId) {
  return '<div style="margin-top:6px;display:flex;align-items:center;gap:6px">'
    + '<input id="rec-' + esc(interactionId) + '" type="url" placeholder="Paste Fathom link…" '
    + 'onkeydown="if(event.key===\'Enter\'){attachRecording(\'' + esc(interactionId) + '\',\'' + esc(dealId) + '\');event.preventDefault()}" '
    + 'style="flex:1;max-width:230px;padding:3px 8px;border:1px solid #d1d5db;border-radius:5px;font-size:11px;font-family:var(--font)">'
    + '<button onclick="attachRecording(\'' + esc(interactionId) + '\',\'' + esc(dealId) + '\')" '
    + 'style="padding:3px 10px;border:none;border-radius:5px;background:#7c3aed;color:#fff;font-size:11px;font-weight:600;cursor:pointer">Attach</button>'
    + '</div>';
}

async function attachRecording(interactionId, dealId) {
  const input = document.getElementById('rec-' + interactionId);
  const url = input && input.value.trim();
  if (!url) return;
  const { sbUpdateInteraction, showToast } = await import('./api.js?v=20260919033445');
  ...
  const item = (_interactionsCache[dealId] || []).find(i => i.id === interactionId);
  if (!item) return;
  // Appended on its own line, exactly as call-touchpoints.js does for JustCall,
  // so the existing linkifier picks it up with no special casing.
  const content = String(item.content || '') + '\n' + url;
  try {
    await sbUpdateInteraction(interactionId, { content });
    item.content = content;
    closeTimelinePanel();
    openTimelinePanel(dealId);
    const preview = document.getElementById('interaction-preview');
    if (preview) preview.innerHTML = buildPreviewHTML(dealId);
    showToast('Recording attached', 'success');
  } catch (e) {
    showToast('Could not attach the recording: ' + e.message, 'error');
  }
}
...
function openTimelinePanel(dealId) {
  ...
  if (all.length === 0) {
    ...
    all.forEach((ev, i) => {
      ...
      // A Gmail-synced row mirrors Gmail, which is the real source of truth —
      // and deleting it here would be futile, because the next poll re-inserts
      // it while the message is still inside the sync's 2-day window. To remove
      // one, delete the email in Gmail.
      const readOnly = ev.system || ev.source === 'gmail';
      if (!readOnly) {
      ...
      if (!readOnly && ev.type === 'Call' && isCallTouchpoint(ev.content) && !hasDisposition(ev.content)) {
        html += renderDispositionPicker(ev.id, dealId);
      }
      // A discovery call that has been and gone, with no outcome recorded on the
      // deal. Asking here — on the booking itself — beats asking in a queue,
      // because the calls around it are right there as evidence.
      if (!readOnly && ev.type === 'Meeting' && needsDiscoOutcome(ev, all)) {
        html += renderDiscoOutcomeButtons(dealId);
      }
      if (!readOnly && ev.type === 'Meeting' && needsDemoOutcome(ev, all)) {
        html += renderDemoOutcomeButtons(dealId);
      }
      if (!readOnly && ev.type === 'Meeting' && isMeetingTouchpoint(ev.content) && !hasRecording(ev.content)) {
        html += renderRecordingAttach(ev.id, dealId);
      }
      html += '</div></div>';
    });
  }
...
async function addInteraction(dealId) {
  ...
  const { sbCreateInteraction } = await import('./api.js?v=20260919033445');
...
async function deleteInteraction(id, dealId) {
  const { sbDeleteInteraction } = await import('./api.js?v=20260919033445');
...
async function editInteractionDate(id, dealId) {
  ...
  const { sbUpdateInteraction } = await import('./api.js?v=20260919033445');
...
window.markDiscoFromTimeline = markDiscoFromTimeline;
window.markDemoFromTimeline = markDemoFromTimeline;
window.attachRecording = attachRecording;
...
window.markAtYourConvenience = async (dealId) => {
  ...
  const { showToast: toast } = await import('./api.js?v=20260919033445');
...
export function renderDealModal(deal){
    ...
    <div class="modal" style="width:520px" onclick="event.stopPropagation()">
      ...
      <div class="modal-body">
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            <label>Stage</label>
            <select id="deal-stage" ${deal._archived?'disabled':''} onchange="updateDealField('stage',this.value)">
  ...
  if(deal.campaignName){
    ...
    h+=`<div class="sl-info">
      ...
      ${deal.pipeline==='Acquisition'?`<div style="margin-top:4px;display:flex;align-items:center;gap:6px">Vertical: <select onchange="updateDealVertical('${esc(deal.id)}',this.value)" style="padding:2px 8px;border:1px solid #e5e7eb;border-radius:4px;font-size:12px;font-family:var(--font);cursor:pointer">
        ${DEAL_VERTICALS.map(v=>`<option value="${v}" ${(deal.vertical||'Other')===v?'selected':''}>${v}</option>`).join('')}
      </select></div>`:''}
  ...
  if((isAdmin()||isEmployee()) && str(deal.slLeadId).trim() && str(deal.slCampaignId).trim()){
    ...
    if(_tc[deal.id]){
      ...
      h+=`<button id="sl-thread-btn-${esc(deal.id)}" class="sl-thread-btn" onclick="event.stopPropagation();var _b=this;_b.disabled=true;_b.innerHTML='Loading...';loadSmartleadThread('${esc(deal.id)}').then(function(ok){if(ok)_b.style.display='none'})">
        ${brandIcon('smartlead',15)} View Email Thread
      </button>`;
    }
  }

  // Gmail history — the conversation after it left SmartLead and moved into a
  // staff inbox. Read live, never stored. Admin + employee only.
  if(isAdmin()||isEmployee()){
    h+=renderGmailSection(deal);
  }
  ...
  if(deal.pipeline==='Client'){
    ...
    if(matchedClient){
      ...
      // Enhanced Client Info panel
      {
        ...
        const pModel=info.billingModel||'';
...
`;
      ...
      // Text Client about the booked meeting (js/booking-sms.js owns all of it)
      h+=renderBookingSmsButton(deal, matchedClient);
  ...
  if(deal.pipeline==='Acquisition'){
    h+=`<div style="margin:0 0 8px 0;display:flex;gap:6px">
      <button class="btn btn-primary"
        ...
        onclick="event.stopPropagation();showDemoCallPicker('${esc(deal.id)}')">
  ...
  // Location Map — show for ANY deal with an address, regardless of pipeline or matched client
  {
    ...
    // Always show a map on every deal modal
    {
      ...
      h += `<div class="sa-map-container">
        ...
        ${(_mc || hasGeo) ? `<button class="sa-enlarge-btn" onclick="event.stopPropagation();openEnlargedMap('${esc(deal.id).replace(/'/g,"\\'")}','${esc(_mc ? _mc.name : '').replace(/'/g,"\\'")}')" title="Enlarge map">
          ⛶ Enlarge
        </button>` : ''}
      </div>`;
  ...
  if(state.showSop){
    ...
    const sopDays=(state.sopSeq==='reactivation')?REACTIVATION_DAYS:getSopDays(deal);
    ...
    h+=`<div class="sop-grid">
      <div style="display:flex;gap:6px;margin-bottom:10px;width:100%">
        ...
        <button class="sop-day-btn" style="flex:1;${tabStyle('reactivation')}" onclick="state.sopSeq='reactivation';refreshModal()">Reactivation</button>`}
      </div>
      ${(seq==='follow-up'||(!isClient&&seq==='reactivation'))?`
        ...
        <button class="sop-day-btn" onclick="doAssignSequenceWithDate('${deal.id}','${day}','${seq}')">${day} <span style="font-weight:400;color:#6ee7b7">(${acts.length})</span></button>`).join("")}`:''}
  ...
  for(const a of pending){
    ...
    h+=`<div class="act-item ${cls} ${actTypeClass(a.type)}">
      ...
      ${a.dueDate?`<input type="date" class="act-date" value="${dd}" title="Change the due date" style="font-size:10px;color:${overdue?'#ef4444':dueToday?'#059669':'#9ca3af'};background:none;border:none;cursor:pointer;padding:0;width:90px" onclick="this.showPicker&&this.showPicker()" onchange="updateActivityDate('${a.id}',this.value)">`:''}
      ${a.dueDate?`<button onclick="pushActivityToGoogleTask('${a.id}')" title="Remind me — adds this to my Google Tasks" style="background:none;border:none;cursor:pointer;padding:0 4px;color:#6b7280;display:inline-flex;align-items:center">${svgIcon('bell',12)}</button>`:''}
  ...
  h+=`${renderDealRetargetHistory(deal.id)}
    <div class="modal-footer">
      <div style="display:flex;gap:8px;align-items:center">
        ${(isAdmin()||isEmployee())&&!deal._archived?`<button class="btn btn-danger" onclick="showArchiveReasonPicker('${deal.id}')">Archive</button>`:''}
...
export function confirmScheduleAndCopy(){
  ...
  const client=findClientForDeal(deal)||state.clients.find(c=>c.name===deal.stage);
  if(client && dateVal){
    import('./calendly.js?v=20260919033445').then(mod=>{
...
window.pushToClientSheet = async function(dealId) {
  ...
  try {
    ...
    btn.disabled = false;
    const { showToast: toast2 } = await import('./api.js?v=20260919033445');
