...
import { state, pendingWrites, pendingDealFields } from './app.js?v=20260916103231';
import { flushRealtimeQueue, showToast } from './api.js?v=20260916103231';
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, REACTIVATION_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, SUPABASE_URL, SUPABASE_ANON_KEY, detectCountry, ACQ_STRATEGY_BOOKERS, ACQ_DEMO_BOOKERS } from './config.js?v=20260916103231';
import { render, refreshModal } from './render.js?v=20260916103231';
import { apiGet, invokeEdgeFunction, sbUpdateDeal, sbGetDealHeavyFields, camelToSnake } from './api.js?v=20260916103231';
import { esc, str, getToday, TODAY, uid, svgIcon, brandIcon, fmtDate, fmtTime12, fmtTimestamp, stripHtml, applyTemplate } from './utils.js?v=20260916103231';
import { DEFAULT_INSTRUCTIONS_TEMPLATE } from './settings.js?v=20260916103231';
import { renderBookingSmsButton } from './booking-sms.js?v=20260916103231';
import { JUSTCALL_DISPOSITIONS, applyDisposition, isCallTouchpoint } from './call-touchpoints.js?v=20260916103231';
import { HELD, NO_SHOW, OUTCOME_PREFIX, DEMO_OUTCOME_PREFIX, markDisco, markDemo, outcomeSelect, demoOutcomeSelect } from './disco-outcome.js?v=20260916103231';
import { renderDealInvoiceButton } from './deal-invoice.js?v=20260916103231';
import { isAdmin, isEmployee, loadAssignableUsers } from './auth.js?v=20260916103231';
import { saveDeal, createDeal, moveDeal, deleteDeal as deleteDealFn } from './deals.js?v=20260916103231';
import { showAcquisitionRemovalPicker } from './removal-reason.js?v=20260916103231';
import { addActivity, assignSequence, getSopDays, renderUpcomingMeetings, generateAppointmentSequence, reschedulePreCallSequence, assignNoShowSequence } from './activities.js?v=20260916103231';
import { addClient, findClientForDeal, lookupClientInfo, isRetainerClient, getWarmCallQA } from './client-info.js?v=20260916103231';
import { getStagesForPipeline } from './dashboard.js?v=20260916103231';
import { renderServiceAreaMap, findPolygonForClient, serviceAreaResults, geocodeCache, geocodeAndCheckDeal } from './maps.js?v=20260916103231';
import { loadSmartleadThread, renderSmartleadThread, renderThreadMessage, toggleFullThread, getThreadCache, openSendToClientPreview, doSendToClientThread } from './threads.js?v=20260916103231';
import { renderGmailSection } from './gmail-threads.js?v=20260916103231';
import { renderPassoffSection, startTranscriptPolling, stopTranscriptPolling } from './passoff.js?v=20260916103231';
import './blooio.js?v=20260916103231';
import './google-task.js?v=20260916103231';
import './demo-tracker.js?v=20260916103231';
import { renderDealRetargetHistory } from './retargeting.js?v=20260916103231';
...
export async function doWonDrop(){
  ...
  if(deal.pipeline==='Acquisition'){
    ...
    const { openWonModal } = await import('./won-modal.js?v=20260916103231');
    openWonModal(deal);
    return;
  }
  ...
  try {
    const { autoPushToTracker } = await import('./email.js?v=20260916103231');
    await autoPushToTracker(deal);
    wonSuccess = true;
  } catch(e){
    console.error('Won drop action failed:', e);
    const { showToast } = await import('./api.js?v=20260916103231');
    showToast('Won action failed: ' + e.message, 'error');
  }

  if(wonSuccess) {
    const { deleteDeal } = await import('./deals.js?v=20260916103231');
...
async function loadInteractions(dealId) {
  try {
    const { sbGetInteractions } = await import('./api.js?v=20260916103231');
...
async function attachRecording(interactionId, dealId) {
  ...
  if (!url) return;
  const { sbUpdateInteraction, showToast } = await import('./api.js?v=20260916103231');
...
async function addInteraction(dealId) {
  ...
  const { sbCreateInteraction } = await import('./api.js?v=20260916103231');
...
async function deleteInteraction(id, dealId) {
  const { sbDeleteInteraction } = await import('./api.js?v=20260916103231');
...
async function editInteractionDate(id, dealId) {
  ...
  const { sbUpdateInteraction } = await import('./api.js?v=20260916103231');
...
window.markAtYourConvenience = async (dealId) => {
  ...
  const { showToast: toast } = await import('./api.js?v=20260916103231');
...
export function confirmScheduleAndCopy(){
  ...
  const client=findClientForDeal(deal)||state.clients.find(c=>c.name===deal.stage);
  if(client && dateVal){
    import('./calendly.js?v=20260916103231').then(mod=>{
...
window.pushToClientSheet = async function(dealId) {
  ...
  try {
    ...
    btn.disabled = false;
    const { showToast: toast2 } = await import('./api.js?v=20260916103231');
