...
import { state, pendingWrites, pendingDealFields } from './app.js?v=20260531g';
import { flushRealtimeQueue } from './api.js?v=20260531g';
import { ACQUISITION_STAGES, NURTURE_STAGES, SOP_DAYS, ACTIVITY_TYPES, ACTIVITY_ICONS, SUPABASE_URL, SUPABASE_ANON_KEY, detectCountry } from './config.js?v=20260531g';
import { render, refreshModal } from './render.js?v=20260531g';
import { apiGet, invokeEdgeFunction, sbUpdateDeal, sbGetDealHeavyFields, camelToSnake } from './api.js?v=20260531g';
import { esc, str, getToday, TODAY, uid, svgIcon, fmtDate, fmtTime12, fmtTimestamp, stripHtml, applyTemplate } from './utils.js?v=20260531g';
import { DEFAULT_INSTRUCTIONS_TEMPLATE } from './settings.js?v=20260531g';
import { isAdmin, isClient, isEmployee, loadAssignableUsers } from './auth.js?v=20260531g';
import { saveDeal, createDeal, moveDeal, deleteDeal as deleteDealFn } from './deals.js?v=20260531g';
import { addActivity, assignSequence, getSopDays, renderUpcomingMeetings, generateAppointmentSequence, reschedulePreCallSequence, assignNoShowSequence } from './activities.js?v=20260531g';
import { addClient, findClientForDeal, lookupClientInfo, isRetainerClient, getWarmCallQA } from './client-info.js?v=20260531g';
import { getStagesForPipeline } from './dashboard.js?v=20260531g';
import { renderServiceAreaMap, findPolygonForClient, serviceAreaResults, geocodeCache, geocodeAndCheckDeal } from './maps.js?v=20260531g';
import { loadSmartleadThread, renderSmartleadThread, renderThreadMessage, toggleFullThread, getThreadCache, openSendToClientPreview, doSendToClientThread } from './threads.js?v=20260531g';
import { renderPassoffSection, startTranscriptPolling, stopTranscriptPolling } from './passoff.js?v=20260531g';
import './blooio.js';
import './demo-tracker.js';
import { renderDealRetargetHistory } from './retargeting.js?v=20260531g';
...
export async function doWonDrop(){
  ...
  try {
    if(deal.pipeline==='Acquisition'){
      const { autoCreateClient } = await import('./client-info.js?v=20260531g');
      const result = await autoCreateClient(deal);
      wonSuccess = true; // Even if user skipped duplicate, still archive
    } else {
      const { autoPushToTracker } = await import('./email.js?v=20260531g');
      await autoPushToTracker(deal);
      wonSuccess = true;
    }
  } catch(e){
    console.error('Won drop action failed:', e);
    const { showToast } = await import('./api.js?v=20260531g');
    showToast('Won action failed: ' + e.message, 'error');
  }

  if(wonSuccess) {
    const { deleteDeal } = await import('./deals.js?v=20260531g');
...
async function loadInteractions(dealId) {
  try {
    const { sbGetInteractions } = await import('./api.js?v=20260531g');
...
async function addInteraction(dealId) {
  ...
  const { sbCreateInteraction } = await import('./api.js?v=20260531g');
...
async function deleteInteraction(id, dealId) {
  const { sbDeleteInteraction } = await import('./api.js?v=20260531g');
...
async function editInteractionDate(id, dealId) {
  ...
  const { sbUpdateInteraction } = await import('./api.js?v=20260531g');
...
export function confirmScheduleAndCopy(){
  ...
  const client=findClientForDeal(deal)||state.clients.find(c=>c.name===deal.stage);
  if(client && dateVal){
    import('./calendly.js?v=20260531g').then(mod=>{
