...
// ═══════════════════════════════════════════════════════════
import { state, store, pendingWrites, pendingDealFields, deletedDealIds } from './app.js?v=20260814054400';
import { render } from './render.js?v=20260814054400';
import { sbCreateDeal, sbUpdateDeal, sbDeleteDeal, sbArchiveDeal, sbRestoreFromArchive, sbCreateActivity, camelToSnake, invokeEdgeFunction } from './api.js?v=20260814054400';
import { clearDashboardArchiveCache } from './dashboard.js?v=20260814054400';
import { uid, getToday, str } from './utils.js?v=20260814054400';
...
export async function moveDeal(dealId,newStage){
  ...
  if(d && (newStage==='Discovery Scheduled' || newStage==='Demo Scheduled') && d.bookedDate && /^\d{4}-\d{2}-\d{2}$/.test(d.bookedDate)){
    const { generateAppointmentSequence } = await import('./activities.js?v=20260814054400');
    generateAppointmentSequence(d);
  }
  if(d && newStage==='No Show'){
    const { assignNoShowSequence } = await import('./activities.js?v=20260814054400');
...
export async function bulkAddActivity(){
  ...
  const { addActivity } = await import('./activities.js?v=20260814054400');
...
export async function bulkRestoreFromArchive(){
  ...
  try{
    ...
    clearDashboardArchiveCache();
    const { initialSync } = await import('./api.js?v=20260814054400');
    initialSync();
  }finally{ pendingWrites.value--; }
}
