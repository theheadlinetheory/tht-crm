...
// ═══════════════════════════════════════════════════════════
import { state, store, pendingWrites, pendingDealFields, deletedDealIds, clientPortalStages } from './app.js?v=20260601b';
import { ACQUISITION_STAGES } from './config.js?v=20260601b';
import { render } from './render.js?v=20260601b';
import { sbCreateDeal, sbUpdateDeal, sbDeleteDeal, sbArchiveDeal, sbRestoreFromArchive, sbCreateActivity, camelToSnake, invokeEdgeFunction } from './api.js?v=20260601b';
import { clearDashboardArchiveCache } from './dashboard.js?v=20260601b';
import { uid, getToday, str } from './utils.js?v=20260601b';
import { isClient, currentUser } from './auth.js?v=20260601b';
...
export async function moveDeal(dealId,newStage){
  ...
  if(d && (newStage==='Discovery Scheduled' || newStage==='Demo Scheduled') && d.bookedDate && /^\d{4}-\d{2}-\d{2}$/.test(d.bookedDate)){
    const { generateAppointmentSequence } = await import('./activities.js?v=20260601b');
    generateAppointmentSequence(d);
  }
  if(d && newStage==='No Show'){
    const { assignNoShowSequence } = await import('./activities.js?v=20260601b');
...
export async function bulkAddActivity(){
  ...
  const { addActivity } = await import('./activities.js?v=20260601b');
...
export async function bulkRestoreFromArchive(){
  ...
  try{
    ...
    const { initialSync } = await import('./api.js?v=20260601b');
    initialSync();
  }finally{ pendingWrites.value--; }
}
