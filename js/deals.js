...
// ═══════════════════════════════════════════════════════════
import { state, store, pendingWrites, pendingDealFields, deletedDealIds, clientPortalStages } from './app.js?v=20260702a';
import { ACQUISITION_STAGES } from './config.js?v=20260702a';
import { render } from './render.js?v=20260702a';
import { sbCreateDeal, sbUpdateDeal, sbDeleteDeal, sbArchiveDeal, sbRestoreFromArchive, sbCreateActivity, camelToSnake, invokeEdgeFunction } from './api.js?v=20260702a';
import { clearDashboardArchiveCache } from './dashboard.js?v=20260702a';
import { uid, getToday, str } from './utils.js?v=20260702a';
import { isClient, currentUser } from './auth.js?v=20260702a';
...
export async function moveDeal(dealId,newStage){
  ...
  if(d && (newStage==='Discovery Scheduled' || newStage==='Demo Scheduled') && d.bookedDate && /^\d{4}-\d{2}-\d{2}$/.test(d.bookedDate)){
    const { generateAppointmentSequence } = await import('./activities.js?v=20260702a');
    generateAppointmentSequence(d);
  }
  if(d && newStage==='No Show'){
    const { assignNoShowSequence } = await import('./activities.js?v=20260702a');
...
export async function bulkAddActivity(){
  ...
  const { addActivity } = await import('./activities.js?v=20260702a');
...
export async function bulkRestoreFromArchive(){
  ...
  try{
    ...
    clearDashboardArchiveCache();
    const { initialSync } = await import('./api.js?v=20260702a');
    initialSync();
  }finally{ pendingWrites.value--; }
}
