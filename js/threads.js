...
import { apiGet, invokeEdgeFunction } from './api.js?v=20260515d';
import { esc } from './utils.js?v=20260515d';
...
export async function openSendToClientPreview(dealId, clientName){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { getClientThreadId, lookupClientInfo } = await import('./client-info.js?v=20260515d');
