...
import { apiPost, invokeEdgeFunction } from './api.js?v=20260908124535';
import { esc } from './utils.js?v=20260908124535';
...
export async function openSendToClientPreview(dealId, clientName){
  const deal=state.deals.find(d=>d.id===dealId);
  if(!deal) return;
  const { getClientThreadId, lookupClientInfo } = await import('./client-info.js?v=20260908124535');
