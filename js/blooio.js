...
// ═══════════════════════════════════════════════════════════
import { state, pendingWrites } from './app.js?v=20260717141537';
import { showToast, sbCreateActivity, sbUpdateDeal, camelToSnake } from './api.js?v=20260717141537';
import { uid, getToday, esc, applyTemplate } from './utils.js?v=20260717141537';
import { refreshModal } from './render.js?v=20260717141537';
import { BLOOIO_BASE_URL, BLOOIO_API_KEY, SEQUENCE_TEMPLATES, CLIENT_LEAD_TEMPLATES, SUPABASE_URL, SUPABASE_ANON_KEY } from './config.js?v=20260717141537';
import { findClientForDeal } from './client-info.js?v=20260717141537';
...
export function openBlooioModal(dealId, phoneField){
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:12px;width:560px;max-width:92vw;box-shadow:0 20px 60px rgba(0,0,0,.3);overflow:hidden;display:flex;flex-direction:column;max-height:90vh">
      ...
      <div style="border-top:1px solid #e5e7eb;padding:12px 16px;background:#fff;flex-shrink:0">
        <div id="blooio-tpl-row" style="display:flex;gap:6px;margin-bottom:8px">
          <select id="blooio-seq" style="flex:1;padding:7px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:12px;font-family:inherit;color:#6b7280;background:#fff">
            <option value="">Sequence...</option>
          </select>
          <select id="blooio-tpl" style="flex:1;padding:7px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:12px;font-family:inherit;color:#6b7280;background:#fff" disabled>
  ...
  const seqSelect = document.getElementById('blooio-seq');
  ...
  if(sequences.length){
    sequences.forEach((seq, i) => {
      ...
      opt.value = i;
      opt.textContent = seq.label;
      seqSelect.appendChild(opt);
    });
    seqSelect.style.color = '#374151';
  } else {
    document.getElementById('blooio-tpl-row').style.display = 'none';
  }

  function populateTemplates(seqIdx){
    tplSelect.innerHTML = '<option value="">Template...</option>';
    if(isNaN(seqIdx) || !sequences[seqIdx]){ tplSelect.disabled = true; tplSelect.style.color = '#6b7280'; return; }
    sequences[seqIdx].templates.forEach((t, i) => {
      const opt = document.createElement('option');
      opt.value = i;
