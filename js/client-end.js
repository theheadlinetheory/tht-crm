...
import { esc, str } from './utils.js?v=20260908123148';
import { supabase } from './api.js?v=20260908123148';
...
export function showClientEndPicker(clientId, { onDone, onCancel } = {}) {
  ...
  // The reason is only the first half. The offboarding flow takes it from here:
  // it captures the client's history, writes the permanent record, then takes
  // them out of the CRM, Drive and Smartlead. It writes these same fields at its
  // "Mark inactive" step, so nothing is saved twice.
  const finish = async (reason, kind) => {
    ...
    div.remove();
    const { openOffboard } = await import('./client-offboard.js?v=20260908123148');
  ...
  END_REASONS.forEach(r => button(r.label, r.kind, () => finish(r.label, r.kind), r.hint));
  button('Other…', 'other', () => { const r = prompt('Reason:'); if (r && r.trim()) finish('Other: ' + r.trim(), 'churn'); });
