...
import { esc, svgIcon } from './utils.js?v=20260925131125';
import { supabase } from './supabase-client.js?v=20260925131125';
import { PERIODS, periods, pinKey, periodRange, fetchPeriod, rangeLabel, rangeDays, todayYmdLA, requestExactSends } from './funnel-period.js?v=20260925131125';
import { leadsTable } from './funnel-leads.js?v=20260925131125';
...
function loadPeriod(key, rerender) {
  ...
  if (!rerender) rerender = () => import('./render.js?v=20260925131125').then(m => m.render());
  _periodLoading = key;
  const g = (_gen[key] = (_gen[key] || 0) + 1);
...
window.setFunnelPeriod = (key) => {
  ...
  import('./render.js?v=20260925131125').then(m => { if (key !== 'all') loadPeriod(key, m.render); m.render(); });
};
...
const rerenderNow = () => import('./render.js?v=20260925131125').then(m => m.render());
...
window.refreshFunnel = () => {
  import('./render.js?v=20260925131125').then(m => reloadFunnel(m.render));
};
