...
import { PERIODS, periods, pinKey, periodRange, fetchPeriod, rangeLabel } from './funnel-period.js?v=20260918075808';
import { leadsTable } from './funnel-leads.js?v=20260918075808';
...
function loadPeriod(key, rerender) {
  ...
  if (!rerender) rerender = () => import('./render.js?v=20260918075808').then(m => m.render());
  _periodLoading = key;
  const g = (_gen[key] = (_gen[key] || 0) + 1);
...
window.setFunnelPeriod = (key) => {
  ...
  import('./render.js?v=20260918075808').then(m => { if (key !== 'all') loadPeriod(key, m.render); m.render(); });
};
...
window.refreshFunnel = () => {
  import('./render.js?v=20260918075808').then(m => reloadFunnel(m.render));
};
