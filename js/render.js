    ...
    if(sd.length===0){
      ...
      for(const deal of sd){
        ...
        html+=`<div class="deal-card${isBulkSel?' bulk-selected':''}${deal.hasNewReply?' has-reply':''}" draggable="${state.bulkMode?'false':'true'}"
          ...
          ${deal.linkedinUrl?`<div class="deal-detail"><a href="${esc(deal.linkedinUrl)}" target="_blank" rel="noopener" onclick="event.stopPropagation()" style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;background:#0a66c2;color:#fff;border-radius:4px;font-size:10px;font-weight:600;text-decoration:none">${svgIcon('external-link',9,'#fff')} LinkedIn</a></div>`:''}
