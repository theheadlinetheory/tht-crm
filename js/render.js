...
import { state, savedScrollLeft, setSavedScrollLeft } from './app.js?v=20260910112031';
import { ACQUISITION_STAGES, NURTURE_STAGES, ACTIVITY_ICONS, detectCountry } from './config.js?v=20260910112031';
import { esc, svgIcon, getToday, fmtDate, fmtTime12, str, stripHtml } from './utils.js?v=20260910112031';
import { isAdmin, isEmployee, isFounder, currentUser, renderUserMenu, getOwnerForDeal, getOwnerNameForDeal, loadAssignableUsers } from './auth.js?v=20260910112031';
import { initialSync as syncFromSheet } from './api.js?v=20260910112031';
import { getStages, getPipelineDeals, getVisiblePipelinesWithArchive, globalSearch, clearSearch, getActivityBadge } from './search.js?v=20260910112031';
import { openDeal, openNewDeal, showDeleteZone, hideDeleteZone, doLostDrop, doWonDrop, renderDealModal, renderNewDealModal, renderAddClientModal, toggleBadgeDropdown } from './deal-modal.js?v=20260910112031';
import { renderOverdueBanner, renderBookedMeetingsBanner, leadAgeBadge } from './activities.js?v=20260910112031';
import { renderDashboard, renderKpiTargetBar } from './dashboard.js?v=20260910112031';
import { loadFunnel, renderFunnel } from './funnel.js?v=20260910112031';
import { loadArchive, renderArchiveTab, toggleViewMode, updateArchiveStatus, restoreFromArchive } from './archive.js?v=20260910112031';
import { toggleBulkMode, bulkMoveStage, bulkSelectAll, bulkArchive, bulkAddActivity, toggleBulkSelect } from './deals.js?v=20260910112031';
import { openSettings } from './settings.js?v=20260910112031';
import { serviceAreaResults } from './maps.js?v=20260910112031';
import { lookupClientInfo, isRetainerClient, deriveTimezone } from './client-info.js?v=20260910112031';
import { openClientInfoPanel } from './client-panel.js?v=20260910112031';
import { openCalendlyEmbed, removeAppointment, addManualAppointment } from './calendly.js?v=20260910112031';
import { capturePreserve, restorePreserve } from './render-preserve.js?v=20260910112031';
import { doDragOver, doDragLeave, clearAllDragOver, doDrop } from './deals.js?v=20260910112031';
import { loadDiscoOutcomes, renderDiscoOutcomeBanner } from './disco-outcome.js?v=20260910112031';
import { renderDueTodayBanner, renderNurtureTab, renderNurtureEntryModal, renderReactivateModal, renderSnoozeModal, loadNurtureData, clearNurtureDraft } from './rerun.js?v=20260910112031';
import { renderDemoTracker } from './demo-tracker.js?v=20260910112031';
import { renderColdCallingTab } from './cold-calling.js?v=20260910112031';
import { renderRetargetingTab } from './retargeting.js?v=20260910112031';
import { renderCacTab } from './cac.js?v=20260910112031';
import { renderRenewals } from './renewals.js?v=20260910112031';
import { isPowerDialerActive } from './power-dialer.js?v=20260910112031';
    ...
    if(clSubTab === 'analysis' && (isAdmin()||isEmployee())){
      if(window._analysisModule){
        ...
        if(!window._analysisLoading){
          ...
          import('./analysis.js?v=20260910112031').then(m=>{ window._analysisModule=m; render(); }).catch(()=>{ window._analysisLoading=false; });
        }
      }
    ...
    if(clSubTab === 'triage' && isAdmin()){
      if(window._triageModule){
        ...
        if(!window._triageLoading){
          ...
          import('./triage.js?v=20260910112031').then(m=>{ window._triageModule=m; render(); }).catch(()=>{ window._triageLoading=false; });
        }
      }
    ...
    if(clSubTab === 'followups' && isAdmin()){
      if(window._followupModule){
        ...
        if(!window._followupLoading){
          ...
          import('./followup-reminders.js?v=20260910112031').then(m=>{ window._followupModule=m; render(); }).catch(()=>{ window._followupLoading=false; });
        }
      }
      app.innerHTML=html;
      return;
    ...
    if(clSubTab === 'weekly_updates' && isAdmin()){
      if(window._weeklyModule){
        ...
        if(!window._weeklyLoading){
          ...
          import('./weekly-updates.js?v=20260910112031').then(m=>{ window._weeklyModule=m; render(); }).catch(()=>{ window._weeklyLoading=false; });
        }
      }
      app.innerHTML=html;
      return;
    ...
    if(clSubTab === 'monthly_updates' && isAdmin()){
      if(window._monthlyModule){
        ...
        if(!window._monthlyLoading){
          ...
          import('./monthly-updates.js?v=20260910112031').then(m=>{ window._monthlyModule=m; render(); }).catch(()=>{ window._monthlyLoading=false; });
        }
      }
      app.innerHTML=html;
      return;
      ...
      if(state.trackerView==='monthly'){
        ...
        if(state.trackerLoaded && window._trackerMonthlyModule){
          ...
          if(!window._trackerMonthlyLoading){
            ...
            import('./lead-tracker-monthly.js?v=20260910112031').then(m=>{ window._trackerMonthlyModule=m; render(); })
              .catch(()=>{ window._trackerMonthlyLoading=false; });
          }
          if(!state.trackerLoaded && !window._trackerLoading){
            window._trackerLoading=true;
            import('./lead-tracker.js?v=20260910112031').then(m=>{
        ...
        if(state.trackerLoaded && window._trendsModule){
          ...
          if(!window._trendsLoading){
            ...
            import('./trends.js?v=20260910112031').then(m=>{ window._trendsModule=m; render(); }).catch(()=>{ window._trendsLoading=false; });
          }
          if(!state.trackerLoaded && !window._trackerLoading){
            window._trackerLoading=true;
            import('./lead-tracker.js?v=20260910112031').then(m=>{
        ...
        if(state.passOffsLoaded && window._passOffsModule){
          ...
          if(!window._passOffsLoading){
            ...
            import('./pass-offs.js?v=20260910112031').then(m=>{ window._passOffsModule=m; render(); }).catch(()=>{ window._passOffsLoading=false; });
          }
        }
      } else {
        if(state.trackerLoaded && window._trackerModule){
          ...
          if(!window._trackerLoading){
            ...
            if(!window._invoiceLoading){
              ...
              import('./invoice.js?v=20260910112031').then(m=>{ window._invoiceModule=m; }).catch(()=>{ window._invoiceLoading=false; });
            }
          }
        }
        if(state.invoiceModal && window._invoiceModule){
          ...
          window._invoiceLoading=true;
          import('./invoice.js?v=20260910112031').then(m=>{ window._invoiceModule=m; render(); });
        }
      }
