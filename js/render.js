...
import { state, savedScrollLeft, setSavedScrollLeft } from './app.js?v=20260922004126';
import { ACQUISITION_STAGES, NURTURE_STAGES, ACTIVITY_ICONS, DEAL_VERTICALS, detectCountry } from './config.js?v=20260922004126';
import { esc, svgIcon, getToday, fmtDate, fmtTime12, str, stripHtml } from './utils.js?v=20260922004126';
import { isAdmin, isEmployee, isFounder, currentUser, renderUserMenu, getOwnerForDeal, getOwnerNameForDeal, loadAssignableUsers } from './auth.js?v=20260922004126';
import { initialSync as syncFromSheet } from './api.js?v=20260922004126';
import { getStages, getPipelineDeals, getVisiblePipelinesWithArchive, globalSearch, clearSearch, getActivityBadge } from './search.js?v=20260922004126';
import { openDeal, openNewDeal, showDeleteZone, hideDeleteZone, doLostDrop, doWonDrop, renderDealModal, renderNewDealModal, renderAddClientModal, toggleBadgeDropdown } from './deal-modal.js?v=20260922004126';
import { renderOverdueBanner, renderBookedMeetingsBanner, leadAgeBadge } from './activities.js?v=20260922004126';
import { renderDashboard, renderKpiTargetBar } from './dashboard.js?v=20260922004126';
import { loadFunnel, renderFunnel } from './funnel.js?v=20260922004126';
import { journeyCell, loadArchive, renderArchiveTab, toggleViewMode, updateArchiveStatus, restoreFromArchive } from './archive.js?v=20260922004126';
import { journeyFor, LEFT_AT_OPTIONS, WHY_OPTIONS } from './archive-journey.js?v=20260922004126';
import { toggleBulkMode, bulkMoveStage, bulkSelectAll, bulkArchive, bulkAddActivity, toggleBulkSelect } from './deals.js?v=20260922004126';
import { openSettings } from './settings.js?v=20260922004126';
import { serviceAreaResults } from './maps.js?v=20260922004126';
import { lookupClientInfo, isRetainerClient, deriveTimezone } from './client-info.js?v=20260922004126';
import { openClientInfoPanel } from './client-panel.js?v=20260922004126';
import { openCalendlyEmbed, removeAppointment, addManualAppointment } from './calendly.js?v=20260922004126';
import { capturePreserve, restorePreserve } from './render-preserve.js?v=20260922004126';
import { doDragOver, doDragLeave, clearAllDragOver, doDrop } from './deals.js?v=20260922004126';
import { loadDiscoOutcomes, renderDiscoOutcomeBanner } from './disco-outcome.js?v=20260922004126';
import { renderDueTodayBanner, renderNurtureTab, renderNurtureEntryModal, renderReactivateModal, renderSnoozeModal, loadNurtureData, clearNurtureDraft } from './rerun.js?v=20260922004126';
import { renderDemoTracker } from './demo-tracker.js?v=20260922004126';
import { renderColdCallingTab } from './cold-calling.js?v=20260922004126';
import { renderRetargetingTab } from './retargeting.js?v=20260922004126';
import { renderCacTab } from './cac.js?v=20260922004126';
import { renderRenewals } from './renewals.js?v=20260922004126';
import { isPowerDialerActive } from './power-dialer.js?v=20260922004126';
...
// ─── render ───
// A full-page re-render destroys every form field in #app. Snapshot what the
// user is editing before the swap and put it back straight after, in the SAME
// synchronous task so no keystroke can land in the gap and be dropped.
export function render(){
  const preserved = capturePreserve(document);
  try { renderInternal(); }
  finally { restorePreserve(document, preserved); }
}

function renderInternal(){
  // An archived deal opened from the Funnel or the Archive is parked in state.deals only while its card is open
  // (archive.js). Every other path that closes the card leaves it there — drop it here so it can never sit on the
  // board, be dragged, or be found by rerun/nurture code (hardening review, 2026-09-15).
  if(state.deals.some(d=>d._archived)) state.deals=state.deals.filter(d=>!d._archived || d===state.selectedDeal);
  try{
  ...
  if (state.pipeline === 'acquisition' && state.acquisitionSubTab === 'cold_calls' && isPowerDialerActive() && !state._pdRenderRequested) return;
  ...
  // ─── Funnel Tab ───
  if(state.pipeline==='funnel'){
    loadFunnel(render);
    app.innerHTML=`
    <div class="topbar">
      <div style="display:flex;align-items:center;">
        <div class="topbar-tabs">${pipelineTabsHtml()}</div>
      </div>
      <div class="topbar-right">${renderUserMenu()}</div>
    </div>`+renderFunnel()+(state.selectedDeal?renderDealModal(state.selectedDeal):''); // this screen returns before the modals below — a lead's card (funnelOpenDeal) has to be added here
    return;
  }

  // ─── Renewals Tab ───
  // Read-only retainer renewal schedule, so the dates can be checked without
  // waiting on the 7/3/1 notices. Admin-only: it shows retainer amounts.
  if(state.pipeline==='renewals'){
    if(!isAdmin()){ state.pipeline='dashboard'; location.hash='dashboard'; render(); return; }
    let html=`
    <div class="topbar">
      <div style="display:flex;align-items:center;">
        <div class="topbar-tabs">${pipelineTabsHtml()}</div>
    ...
    // render() is one big try/catch, so an unguarded throw in here would abort
    // before app.innerHTML is assigned and silently leave the previous screen up.
    try{
      html+=renderRenewals();
    }catch(err){
      console.error('Renewals render failed:', err);
      html+=`<div style="padding:32px;text-align:center">
        <div style="font-size:14px;font-weight:700;color:#b91c1c;margin-bottom:6px">The Renewals tab hit an error</div>
        <div style="font-size:12px;color:var(--text-muted);max-width:560px;margin:0 auto">${esc(str(err&&err.message)||'Unknown error')}</div>
      </div>`;
    }
    app.innerHTML=html;
    return;
  ...
  ${(() => {
    ...
    // Level 03's manual step: which booked Meet discos actually happened.
    let disco = '';
    if(state.pipeline==='acquisition'){ loadDiscoOutcomes(render); disco = renderDiscoOutcomeBanner(); }
    const parts = [overdue, booked, nurture, disco].filter(Boolean);
  ...
  if(state.pipeline==='client_leads' && !state.showEmployeeArchive){
    ...
    html += `<div style="padding:0 20px;margin-bottom:8px;display:flex;align-items:center">
      <div style="display:flex;gap:4px">
        ...
        ${isAdmin()?`<button onclick="switchClientLeadsTab('triage')" style="${subCs};background:${clSubTab==='triage'?'var(--purple)':'#f3f4f6'};color:${clSubTab==='triage'?'#fff':'var(--text-muted)'}">Client Triage</button>`:''}
      </div>
    </div>`;

    // Analysis — why clients missed their weekly KPI (lazy: pulls live Smartlead stats)
    if(clSubTab === 'analysis' && (isAdmin()||isEmployee())){
      if(window._analysisModule){
        // Fail INSIDE the tab. render() is one big try/catch, so a throw here
        // used to abort before `app.innerHTML = html` and simply leave the
        // previous screen up — which reads as "the Analysis tab bounced me back
        // to Pipeline", with no error anywhere the user can see.
        let analysisHtml;
        try{
          analysisHtml = window._analysisModule.renderAnalysis();
        }catch(err){
          console.error('Analysis render failed:', err);
          analysisHtml = `<div class="tracker-container"><div style="padding:32px;text-align:center">
            <div style="font-size:14px;font-weight:700;color:#b91c1c;margin-bottom:6px">The Analysis tab hit an error</div>
            <div style="font-size:12px;color:var(--text-muted);max-width:560px;margin:0 auto 14px">${esc(str(err&&err.message)||'Unknown error')}</div>
            <button class="btn btn-primary" style="font-size:12px;padding:7px 16px" onclick="analysisReset()">Clear saved results and start over</button>
          </div></div>`;
        }
        // --tracker-top compensates .tracker-container's fixed height for the
        // top bar + the Client Leads sub-tab row above it.
        html+='<div style="--tracker-top:96px">'+analysisHtml+'</div>';
      } else {
        ...
        if(!window._analysisLoading){
          ...
          import('./analysis.js?v=20260922004126').then(m=>{ window._analysisModule=m; render(); }).catch(()=>{ window._analysisLoading=false; });
        }
      }
    ...
    // Client Triage — snapshot of triage + health-score inputs, sectioned by
    // the four business categories. Admin-only: it shows retainer amounts and
    // renewal dates. Same fail-inside-the-tab pattern as Analysis above, so a
    // bad snapshot shows an error here instead of silently bouncing to Pipeline.
    if(clSubTab === 'triage' && isAdmin()){
      if(window._triageModule){
        let triageHtml;
        try{
          triageHtml = window._triageModule.renderTriage();
        }catch(err){
          console.error('Client Triage render failed:', err);
          triageHtml = `<div class="tracker-container"><div style="padding:32px;text-align:center">
            <div style="font-size:14px;font-weight:700;color:#b91c1c;margin-bottom:6px">The Client Triage tab hit an error</div>
            <div style="font-size:12px;color:var(--text-muted);max-width:560px;margin:0 auto">${esc(str(err&&err.message)||'Unknown error')}</div>
          </div></div>`;
        }
        html+='<div style="--tracker-top:96px">'+triageHtml+'</div>';
      } else {
        ...
        if(!window._triageLoading){
          ...
          import('./triage.js?v=20260922004126').then(m=>{ window._triageModule=m; render(); }).catch(()=>{ window._triageLoading=false; });
        }
      }
      const tWrap=document.querySelector('.tracker-table-wrap');
      const tTop=tWrap?tWrap.scrollTop:0;
      const tLeft=tWrap?tWrap.scrollLeft:0;
      app.innerHTML=html;
      const newTWrap=document.querySelector('.tracker-table-wrap');
      if(newTWrap){ newTWrap.scrollTop=tTop; newTWrap.scrollLeft=tLeft; }
      return;
    }
    ...
    if(clSubTab === 'followups' && isAdmin()){
      if(window._followupModule){
        ...
        if(!window._followupLoading){
          ...
          import('./followup-reminders.js?v=20260922004126').then(m=>{ window._followupModule=m; render(); }).catch(()=>{ window._followupLoading=false; });
        }
      }
      app.innerHTML=html;
      return;
    ...
    if(clSubTab === 'weekly_updates' && isAdmin()){
      if(window._weeklyModule){
        html+=window._weeklyModule.renderWeeklyUpdates();
      } else {
        ...
        if(!window._weeklyLoading){
          ...
          import('./weekly-updates.js?v=20260922004126').then(m=>{ window._weeklyModule=m; render(); }).catch(()=>{ window._weeklyLoading=false; });
        }
      }
      app.innerHTML=html;
      return;
    ...
    if(clSubTab === 'monthly_updates' && isAdmin()){
      if(window._monthlyModule){
        ...
        if(!window._monthlyLoading){
          ...
          import('./monthly-updates.js?v=20260922004126').then(m=>{ window._monthlyModule=m; render(); }).catch(()=>{ window._monthlyLoading=false; });
        }
      }
      app.innerHTML=html;
      return;
    ...
    if(clSubTab === 'lead_tracker' && (isAdmin()||isEmployee())){
      ...
      html+=`<div style="display:flex;gap:0;border-bottom:1px solid var(--border);padding:0 12px">
        ...
        <button class="topbar-tab ${state.trackerView==='monthly'?'active':''}" onclick="switchTrackerView('monthly')">Monthly</button>
      ...
      if(state.trackerView==='monthly'){
        // Needs the tracker rows loaded before it can count anything.
        if(state.trackerLoaded && window._trackerMonthlyModule){
          html+=window._trackerMonthlyModule.renderMonthly();
        } else {
        ...
        if(state.trackerLoaded && window._trendsModule){
          ...
          if(!window._trendsLoading){
            ...
            import('./trends.js?v=20260922004126').then(m=>{ window._trendsModule=m; render(); }).catch(()=>{ window._trendsLoading=false; });
          }
          if(!state.trackerLoaded && !window._trackerLoading){
            window._trackerLoading=true;
            import('./lead-tracker.js?v=20260922004126').then(m=>{
      ...
      } else if(state.trackerView==='passoffs'){
        if(state.passOffsLoaded && window._passOffsModule){
          ...
          if(!window._passOffsLoading){
            ...
            import('./pass-offs.js?v=20260922004126').then(m=>{ window._passOffsModule=m; render(); }).catch(()=>{ window._passOffsLoading=false; });
          }
        }
      } else {
        if(state.trackerLoaded && window._trackerModule){
          ...
          if(!window._trackerLoading){
            ...
            if(!window._invoiceLoading){
              ...
              import('./invoice.js?v=20260922004126').then(m=>{ window._invoiceModule=m; }).catch(()=>{ window._invoiceLoading=false; });
            }
          }
        }
        if(state.invoiceModal && window._invoiceModule){
          ...
          window._invoiceLoading=true;
          import('./invoice.js?v=20260922004126').then(m=>{ window._invoiceModule=m; render(); });
        }
      }
  ...
  if((isEmployee()||isAdmin()) && state.showEmployeeArchive){
    ...
    // This screen paints itself and returns before the modals below — so an archived deal's card (openArchivedDeal) has to be added here too.
      ...
      for(const deal of sd){
        ...
        html+=`<div class="deal-card${isBulkSel?' bulk-selected':''}${deal.hasNewText?' has-text-reply':''}${deal.hasNewReply?' has-reply':''}" draggable="${state.bulkMode?'false':'true'}"
          ...
          <div class="deal-card-top">
            <div class="deal-company">${state.bulkMode?`<span class="bulk-check">${isBulkSel?'✓':''}</span>`:''}${(()=>{const c=detectCountry(deal);return c.code!=='US'?'<span title="'+c.label+'" style="font-size:12px;margin-right:3px;vertical-align:middle">'+c.flag+'</span>':'';})()}${deal.hasNewText?'<span class="reply-indicator text-reply-indicator" title="New text reply received">'+svgIcon('message-circle',12,'#16a34a')+'</span>':''}${deal.hasNewReply?'<span class="reply-indicator" title="New email reply received">'+svgIcon('mail',12,'#3b82f6')+'</span>':''}${esc(deal.company||deal.contact||deal.email||"New Deal")}${isRetainerClient(deal)?'<span style="display:inline-block;margin-left:6px;font-size:9px;font-weight:700;background:#dbeafe;color:#1d4ed8;padding:1px 5px;border-radius:3px;vertical-align:middle;white-space:nowrap;letter-spacing:.3px">RETAINER</span>':''}${deal.retargetStatus==='replied'?'<span title="Replied to a reactivation campaign" style="display:inline-block;margin-left:6px;font-size:9px;font-weight:700;background:#fef3c7;color:#b45309;padding:1px 5px;border-radius:3px;vertical-align:middle;white-space:nowrap;letter-spacing:.3px">REACTIVATED</span>':''}</div>
          ...
          <div class="deal-bottom">
            <span style="display:flex;align-items:center;gap:4px">
              ...
              ${leadAgeBadge(deal)}
              ${deal.autoFollowupStartedAt?`<span style="font-size:9px;font-weight:700;color:#6d28d9;background:#f3e8ff;padding:1px 5px;border-radius:3px;letter-spacing:.3px;white-space:nowrap">AUTO F/U</span>`:''}
  ...
  // Focus and caret are restored by render()'s restorePreserve() wrapper —
  // it keeps the real caret position instead of jamming it to the end.
...
function switchPipeline(id){
  state.pipeline=id;
  location.hash=id;
  // Clear any stuck nurture modals (and the draft that belonged to them)
...
// ─── Client Leads sub-tab switching ───
// Mirrors the choice into the URL so a reload — including the one a new deploy
// triggers automatically — comes back to the same sub-tab instead of Pipeline.
function switchClientLeadsTab(tab){
  state.clientLeadsSubTab = tab;
  location.hash = tab === 'pipeline' ? 'client_leads' : 'client_leads/' + tab;
  render();
}
window.switchClientLeadsTab = switchClientLeadsTab;
