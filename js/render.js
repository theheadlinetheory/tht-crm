    ...
    html+=`<div class="column" data-stage-key="${stageKey}"
      ...
      <div class="col-header" style="border-top-color:${stage.color}${state.pipeline==='client_leads'&&state.clients.some(c=>c.name===stage.label)?';cursor:pointer':''}" ${state.pipeline==='client_leads'&&state.clients.some(c=>c.name===stage.label)?`onclick="openClientInfoPanel(atob('${btoa(unescape(encodeURIComponent(stage.label)))}'))"`:''}>
        <div>
          ...
          <div class="col-meta">${sd.length} lead${sd.length!==1?'s':''}${(()=>{if(state.pipeline!=='client_leads') return '';const _cl=state.clients.find(c=>c.name===stage.label);if(!_cl) return '';const contactFull=[str(_cl.contactFirstName).trim(),str(_cl.contactLastName).trim()].filter(Boolean).join(' ');const st=str(_cl.clientStanding).toLowerCase()||'neutral';const standingMap={happy:{color:'#22c55e',bg:'#f0fdf4',label:'Happy',rule:'Pass all tiers'},neutral:{color:'#eab308',bg:'#fefce8',label:'Neutral',rule:'A & B freely, limit C\'s'},unhappy:{color:'#ef4444',bg:'#fef2f2',label:'Unhappy',rule:'A & B only, hold C\'s'}};const s=standingMap[st];return (contactFull?` · <span style="font-size:10px;color:#64748b">${esc(contactFull)}</span>`:'')+(s?` <span title="${s.rule}" style="font-size:9px;font-weight:700;color:${s.color};background:${s.bg};padding:1px 6px;border-radius:8px;letter-spacing:.3px;vertical-align:middle;cursor:help;border:1px solid ${s.color}33">${s.label}</span>`:'');})()}</div>
        </div>
