...
import { state, savedScrollLeft, setSavedScrollLeft, clientArchivedDeals } from './app.js?v=20260601b';
import { ACQUISITION_STAGES, NURTURE_STAGES, ACTIVITY_ICONS, detectCountry } from './config.js?v=20260601b';
import { esc, svgIcon, getToday, fmtDate, fmtTime12, str, stripHtml } from './utils.js?v=20260601b';
import { isAdmin, isClient, isEmployee, currentUser, renderUserMenu, getOwnerForDeal, getOwnerNameForDeal, loadAssignableUsers } from './auth.js?v=20260601b';
import { initialSync as syncFromSheet } from './api.js?v=20260601b';
import { getStages, getPipelineDeals, getVisiblePipelinesWithArchive, globalSearch, clearSearch, getActivityBadge } from './search.js?v=20260601b';
import { openDeal, openNewDeal, showDeleteZone, hideDeleteZone, doLostDrop, doWonDrop, renderDealModal, renderNewDealModal, renderAddClientModal, toggleBadgeDropdown } from './deal-modal.js?v=20260601b';
import { renderOverdueBanner, renderBookedMeetingsBanner, leadAgeBadge } from './activities.js?v=20260601b';
import { renderDashboard } from './dashboard.js?v=20260601b';
import { loadArchive, renderArchiveTab, toggleViewMode, updateArchiveStatus, restoreFromArchive } from './archive.js?v=20260601b';
import { renderDocumentsSection, initDocumentHandlers } from './documents.js?v=20260601b';
import { toggleBulkMode, bulkMoveStage, bulkSelectAll, bulkArchive, bulkAddActivity, toggleBulkSelect } from './deals.js?v=20260601b';
import { openSettings } from './settings.js?v=20260601b';
import { serviceAreaResults } from './maps.js?v=20260601b';
import { lookupClientInfo, isRetainerClient, openClientInfoPanel, removeClient, deriveTimezone } from './client-info.js?v=20260601b';
import { openCalendlyEmbed, removeAppointment, addManualAppointment } from './calendly.js?v=20260601b';
import { doDragOver, doDragLeave, clearAllDragOver, doDrop } from './deals.js?v=20260601b';
import { renderDueTodayBanner, renderNurtureTab, renderNurtureEntryModal, renderReactivateModal, renderSnoozeModal, loadNurtureData } from './rerun.js?v=20260601b';
import { renderDemoTracker } from './demo-tracker.js?v=20260601b';
import { renderRetargetingTab } from './retargeting.js?v=20260601b';
...
export function render(){
  ...
  if(state.pipeline==='payroll'){
    if(!window._payrollModule && !window._payrollLoading){
      ...
      import('./payroll.js?v=20260601b').then(m=>{ window._payrollModule=m; if(state.pipeline==='payroll') render(); }).catch(()=>{ window._payrollLoading=false; });
    }
      ...
      if(state.trackerView==='trends'){
        if(state.trackerLoaded && window._trendsModule){
          ...
          if(!window._trendsLoading){
            ...
            import('./trends.js?v=20260601b').then(m=>{ window._trendsModule=m; render(); }).catch(()=>{ window._trendsLoading=false; });
          }
          if(!state.trackerLoaded && !window._trackerLoading){
            window._trackerLoading=true;
            import('./lead-tracker.js?v=20260601b').then(m=>{
        ...
        if(state.trackerLoaded && window._trackerModule){
          ...
          if(!window._trackerLoading){
            ...
            if(!window._invoiceLoading){
              ...
              import('./invoice.js?v=20260601b').then(m=>{ window._invoiceModule=m; }).catch(()=>{ window._invoiceLoading=false; });
            }
          }
        }
        if(state.invoiceModal && window._invoiceModule){
          ...
          window._invoiceLoading=true;
          import('./invoice.js?v=20260601b').then(m=>{ window._invoiceModule=m; render(); });
        }
      }
