...
import { state, savedScrollLeft, setSavedScrollLeft, clientArchivedDeals } from './app.js?v=20260531c';
import { ACQUISITION_STAGES, NURTURE_STAGES, ACTIVITY_ICONS, detectCountry } from './config.js?v=20260531c';
import { esc, svgIcon, getToday, fmtDate, fmtTime12, str, stripHtml } from './utils.js?v=20260531c';
import { isAdmin, isClient, isEmployee, currentUser, renderUserMenu, getOwnerForDeal, getOwnerNameForDeal, loadAssignableUsers } from './auth.js?v=20260531c';
import { initialSync as syncFromSheet } from './api.js?v=20260531c';
import { getStages, getPipelineDeals, getVisiblePipelinesWithArchive, globalSearch, clearSearch, getActivityBadge } from './search.js?v=20260531c';
import { openDeal, openNewDeal, showDeleteZone, hideDeleteZone, doLostDrop, doWonDrop, renderDealModal, renderNewDealModal, renderAddClientModal, toggleBadgeDropdown } from './deal-modal.js?v=20260531c';
import { renderOverdueBanner, renderBookedMeetingsBanner, leadAgeBadge } from './activities.js?v=20260531c';
import { renderDashboard } from './dashboard.js?v=20260531c';
import { loadArchive, renderArchiveTab, toggleViewMode, updateArchiveStatus, restoreFromArchive } from './archive.js?v=20260531c';
import { renderDocumentsSection, initDocumentHandlers } from './documents.js?v=20260531c';
import { toggleBulkMode, bulkMoveStage, bulkSelectAll, bulkArchive, bulkAddActivity, toggleBulkSelect } from './deals.js?v=20260531c';
import { openSettings } from './settings.js?v=20260531c';
import { serviceAreaResults } from './maps.js?v=20260531c';
import { lookupClientInfo, isRetainerClient, openClientInfoPanel, removeClient, deriveTimezone } from './client-info.js?v=20260531c';
import { openCalendlyEmbed, removeAppointment, addManualAppointment } from './calendly.js?v=20260531c';
import { doDragOver, doDragLeave, clearAllDragOver, doDrop } from './deals.js?v=20260531c';
import { renderDueTodayBanner, renderNurtureTab, renderNurtureEntryModal, renderReactivateModal, renderSnoozeModal, loadNurtureData } from './rerun.js?v=20260531c';
import { renderDemoTracker } from './demo-tracker.js?v=20260531c';
import { renderRetargetingTab } from './retargeting.js?v=20260531c';
      ...
      if(state.trackerView==='trends'){
        if(state.trackerLoaded && window._trendsModule){
          ...
          if(!window._trendsLoading){
            ...
            import('./trends.js?v=20260531c').then(m=>{ window._trendsModule=m; render(); }).catch(()=>{ window._trendsLoading=false; });
          }
          if(!state.trackerLoaded && !window._trackerLoading){
            window._trackerLoading=true;
            import('./lead-tracker.js?v=20260531c').then(m=>{
        ...
        if(state.trackerLoaded && window._trackerModule){
          ...
          if(!window._trackerLoading){
            ...
            if(!window._invoiceLoading){
              ...
              import('./invoice.js?v=20260531c').then(m=>{ window._invoiceModule=m; }).catch(()=>{ window._invoiceLoading=false; });
            }
          }
        }
        if(state.invoiceModal && window._invoiceModule){
          ...
          window._invoiceLoading=true;
          import('./invoice.js?v=20260531c').then(m=>{ window._invoiceModule=m; render(); });
        }
      }
