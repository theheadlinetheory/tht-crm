...
import { sbListFolders, sbCreateFolder, sbUpdateFolder, sbDeleteFolder,
         ...
         sbUploadFile, sbDeleteFile, sbGetSignedUrl, showToast } from './api.js?v=20260602a';
import { esc, str, uid, svgIcon } from './utils.js?v=20260602a';
import { isAdmin, isClient, isEmployee, currentUser } from './auth.js?v=20260602a';
...
async function refreshDocsSection(clientId) {
  ...
  if (!container) return;
  const { state } = await import('./app.js?v=20260602a');
