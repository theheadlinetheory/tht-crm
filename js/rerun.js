...
// ═══════════════════════════════════════════════════════════
import { state, store, pendingWrites } from './app.js?v=20260925132239';
import { render } from './render.js?v=20260925132239';
import { sbGetRerunQueue, sbAddToRerun, sbUpdateRerunItem, sbUpdateRerunStatus, sbUpdateDeal, sbUpdateActivity, sbArchiveDeal, sbDeleteDeal, camelToSnake, normalizeRow, invokeEdgeFunction } from './api.js?v=20260925132239';
import { esc, getToday, fmtDate, svgIcon } from './utils.js?v=20260925132239';
import { registerActions } from './delegate.js?v=20260925132239';
import { statCard, filterSelect, modalWrap, modalHeader, modalFooter } from './html-helpers.js?v=20260925132239';
import { NURTURE_NOT_NOW_SEQUENCE, ACQUISITION_STAGES, MANUAL_OUTREACH_OWNER } from './config.js?v=20260925132239';
import { isAdmin, getOwnerNameForDeal, getOwnerColor, loadAssignableUsers } from './auth.js?v=20260925132239';
import { dealHadDemo } from './demo-tracker.js?v=20260925132239';
import { loadJourneys, journeyFor } from './archive-journey.js?v=20260925132239';
  ...
  body += `<div class="modal-body">
    ...
    <div style="margin-bottom:12px">
      ...
      <select id="reactivate-stage" style="width:100%;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:12px;font-family:var(--font)">
        ${ACQUISITION_STAGES.map(s => `<option value="${esc(s.id)}"${s.id==='Reactivating'?' selected':''}>${esc(s.label)}</option>`).join('')}
      </select>
    </div>
  </div>`;
...
registerActions({
  ...
  async confirmReactivate() {
    ...
    const stage = stageEl ? stageEl.value : 'Reactivating';
